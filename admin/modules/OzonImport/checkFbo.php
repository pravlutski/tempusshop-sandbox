...
class checkFBO{
  ...
  public function checkFBOStock()
  {
		...
		$o_id = $this->metric->OzonBase($dataMetrica);
		//
	  $this->db->Query("DELETE FROM wdhs_ozon_fbo_price WHERE 1=1", false, $err_mess.__LINE__);
	  $this->db->Query("DELETE FROM wdhs_ozon_fbo_stock WHERE 1=1", false, $err_mess.__LINE__);
		$this->db->Query("DELETE FROM wdhs_ozon_fbo_sebes WHERE 1=1", false, $err_mess.__LINE__);
		//print_r($this->stockFbo);
		$com = $this->mcom / 100;
		$allCom = $com;
			...
			if (isset($this->items[$key])) {
				...
				if ($value != 0 && $value >= 1) {
					if (isset($this->fromMS[$this->items[$key]['ARTICLE']])) {
						...
						$this->answer[$this->items[$key]['ARTICLE']]['answ'] = '';

						if (isset($this->FBO_WH[$this->items[$key]['OZON_ARTICLE']])) {
							$this->answer[$this->items[$key]['ARTICLE']]['answ'] .= 'Товар числиться на СКЛАДАХ: ';
							foreach ($this->FBO_WH[$this->items[$key]['OZON_ARTICLE']] as $kkk => $value) {
								$this->answer[$this->items[$key]['ARTICLE']]['answ'] .= $value['warehouse_name'].' ';
							}
							$this->answer[$this->items[$key]['ARTICLE']]['answ'] .= '<br>Передаем остаток 0<br><br>';
							$in = array(
								"article" => "'".$this->items[$key]['ARTICLE']."'",
							);
							$this->db->Insert("wdhs_ozon_fbo_stock", $in, $err_mess.__LINE__);
						}

						$newprice = self::getNewPrice($this->items[$key]['ARTICLE'],$this->items[$key]['BRAND_ID'],$this->fromMS[$this->items[$key]['ARTICLE']]);
						$oldprice = $newprice;
						//$newprice = round($newprice - ($newprice * ((($newprice - ($newprice * $newCom)) - ($newprice - ($newprice * $com)))/($newprice - ($newprice * $newCom)))));
						$newprice = round(($newprice * (1 - $allCom)) / (1 - $newCom));
						$this->answer[$this->items[$key]['ARTICLE']]['count']  = $value;
						$this->answer[$this->items[$key]['ARTICLE']]['answ'] .= "Товар <b>{$this->items[$key]['ARTICLE']}</b><br>
						...
						$this->answer[$this->items[$key]['ARTICLE']]['answ'] .= "= round(($oldprice * (1 - $allCom)) / (1 - $newCom))<hr><br><br>";
						...
						$this->db->Insert("wdhs_ozon_fbo_sebes", $in, $err_mess.__LINE__);

						$in = array(
							"article" => "'".$this->items[$key]['ARTICLE']."'",
							"price" => "'".$newprice."'",
						);
						$this->db->Insert("wdhs_ozon_fbo_price", $in, $err_mess.__LINE__);
	...
	public function PrintResult()
	{
		...
		// 	print_r($value);
		// }
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport/logs/fbo/log.txt", print_r(json_encode($this->answer), true).PHP_EOL);
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/panel/engine/ozon/logs/IP/fbo/log.txt", print_r(json_encode($this->answer), true).PHP_EOL);
	}
