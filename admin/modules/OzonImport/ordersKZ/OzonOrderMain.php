...
class OzonOrderMain {
  ...
  public function __construct()
  {
      ...
      $this->options = [
        ...
        'user_id' => 168398, //ID пользователя, от имени которого будут создаваться заказы в битриксе (fbs ozon kz)
        'site_id' => 's4', // ID сайта (tempusshop.ru)
        ...
        'status_table_name' => 'wdhs_ozon_order_status_kz', // Таблица с соответствиями статусов
        'order_table_name' => 'wdhs_ozon_orders_kz', // Таблица с данными о заказах
        'products_table_name' => 'wdhs_ozon_order_products_kz', // Таблица с данными о товарах из заказов
        'customer_table_name' => 'wdhs_ozon_order_customers_kz', // Таблица с данными о покупателях
        'log_path' => '/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport/logs/ordersKZ/' . date('Y-m-d') . '_orders.txt'
      ];
  ...
  public function processOrders():void
  {
    ...
    foreach ( $this->ordersApi as &$orderApi ){
      ...
      //   'address_tail' => 'Финляндия, Хельсинки, Пролетарская 15-88',
      //   'country' => 'Финляндия',
      //   'region' => 'Финляндия',
      //   'pvz_code' => '1723',
      //   'city' => 'Хельсинки',
      ...
      if ( $this->checkIfOrderExists($orderApi) ){
        ...
        if ( $orderApi['status'] != 'awaiting_packaging' && $orderApi['status'] != 'awaiting_registration' ){
