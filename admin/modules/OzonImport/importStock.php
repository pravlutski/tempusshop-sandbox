...
class OzonImportStocks{
    ...
    $this->UploadStocks();
		...
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport/logs/stocks/control/$date.txt", print_r(json_encode($controlLog), true).PHP_EOL,FILE_APPEND);
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport/logs/stocks/control/$date.txt", print_r('#SPLIT#',true).PHP_EOL,FILE_APPEND);
  ...
  public function BuildArray() {
    ...
    foreach ($this->items as $key => &$arItem) {
		...
		$this->db->Update("wdhs_ozon_upload_status", array("percent" => '21'), "WHERE agent='stock'", $err_mess.__LINE__);
	}
