...
$action_id = $_GET['ID'];
$tmp = file_get_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/sales/log.txt');
...
<?php foreach ($log as $key => $value): ?>
  ...
  switch ($key) {
    case 'STAY':
        $name = "ИДЕАЛЬНЫЕ УСЛОВИЯ. Остальсь в акции без изменний";
        $bg = "background-color: rgba(212, 242, 64, 0.44);";
        break;
    case 'STAY-UP':
        $name = "Остальсь в акции. Увеличели цену в акции";
        $bg = "background-color: rgba(212, 242, 64, 0.44);";
        break;
    case 'NOT_ADD':
        $name = "Не добавлены";
