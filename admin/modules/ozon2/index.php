...
$strSql = "SELECT * FROM wdhs_ozon_upload_status_new";
$results = $DB->Query($strSql, false, $err_mess.__LINE__);
while ($row = $results->Fetch()){
  $arLast[$row['agent']] = $row;
}

$tmp = file_get_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/prices/shows/'.$curdate.'.txt');
...
// print_R($arResult['TIME_PRICES']);
// die();
$tmp = file_get_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/stocks/control/'.$curdate.'.txt');
$arTmp = explode('#SPLIT#',$tmp);
foreach ($arTmp as $key => $value) {
  $arSoloLog = json_decode($value,true);
  //print_r($arSoloLog);
  $models = array();
  foreach ($arSoloLog as $key => $value) {
      $timestart = $key;
      $models = $value;
      break;
  }
  $arResult['TIME_STOCK'][] = $timestart;
  if (is_array($value)) {
    $arStock[] = count($models);
  }
}
?>
...
<div class="card">
  ...
  <ul class="list-group list-group-flush">
    <?if ($arLast['price']['status'] == 'COMPLETE'){?>
      <li class="list-group-item resize" style="justify-content: space-between;"><span><b style="margin-right:10px;">Цены: </b>Выгрузка завершена в <?=$arLast['price']['time']?></span><a href="/admin/modules/ozon2/logs/price.php" class="card-link">Лог выгрузки цен</a></li>
    ...
    <?if ($arLast['stock']['status'] == 'COMPLETE'){?>
      <li class="list-group-item resize" style="justify-content: space-between;"><span><b style="margin-right:10px;">Остатки: </b>Выгрузка завершена в <?=$arLast['stock']['time']?></span><a href="/admin/modules/ozon2/logs/stock.php" class="card-link">Лог выгрузки остатков</a></li>
    ...
    <?if ($arLast['products']['status'] == 'COMPLETE'){?>
      <li class="list-group-item resize" style="justify-content: space-between;"><span><b style="margin-right:10px;">Товары: </b>Выгрузка завершена в <?=$arLast['products']['time']?></span><a href="/admin/modules/ozon2/logs/products.php" class="card-link">Лог выгрузки товаров</a></li>
...
<div class="row" style="">
  ...
  <div class="col-md-6 col-sm-12">
    <i style="text-align: right;display:block"><?print_r( 'Последнее обновление было ' . date('Y.m.d G:i:s', filectime('/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/stat/stock.txt')) );?></i>
    <div id="stats-graph">
...
<div class="row" >
  <div class="col-md-6 col-sm-12">
    ...
    <div class="detail-block py-2 pl-4">
      ...
      <form id="settings-period-form" action="" style="margin-top:20px;">
  ...
  <?php
  $perHour = json_decode( file_get_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/modules/ozon2/stat/perHour.json'), 1 );
  $timePostings = $perHour['date'];
  unset($perHour['date']);
   ?>
  <div class="col-md-6 col-sm-12">
    <div style="margin-right:auto; margin-left:auto; width: 400px; padding-top: 40px" class="per-hour-block">
      <span style="font-size: 14px; opacity: 0.5; text-align:center; display: block"><i class="last-update-share">Последнее обновление: <?echo $timePostings;?></i></span>
    ...
    <div class="row" style="margin-bottom: 20px; padding-right: 20px;">
      <button id="update-shares" class="btn btn-warning" style="width: fit-content; margin-left:auto">Обновить</button>
    </div>
  </div>
</div>
<?
...
*/
$strSql = "SELECT * FROM ozon_stock_fbo_stat_ti";
...
FROM `ozon_postings_shares_ti`
WHERE date BETWEEN '{$minDate}' AND '{$maxDate}'
GROUP BY date,type";
$strSql = "SELECT date,type, count(model) AS count FROM ozon_postings_shares_ti GROUP BY date,type";
// var_dump($strSql);
$resultDB = $DB->Query($strSql, false, $err_mess.__LINE__);
$postingsRaw = [];
while ( $row = $resultDB->Fetch() ){
...
function showStatByPeriod(flag = false){
  ...
  $.ajax({
    url: '/admin/modules/ozon2/ajax/get_stock_stat_by_period.php',
    method: 'post',
    data: data,
...
$(document).on('click','#show-period',function(e){
  e.preventDefault();
  $.ajax({
    url: '/admin/modules/ozon2/ajax/changePeriodPostings.php',
    method: 'post',
...
$(document).on('click', '#update-shares', function(){
  // Сорри за этот ужас
  $.ajax({
    url: '/admin/modules/ozon2/cron/getPostingsStatPH.php',
    method: 'POST',
    success: function(response){
      console.log('ok');
      $.ajax({
        url: '/admin/modules/ozon2/ajax/updateOrderShares.php',
        method: 'POST',
        success: function(data){
          var prep = $.parseJSON(data)
          console.log( Object.keys(prep) );
          $('#postings_per_hour').remove();
          $('.per-hour-block').html('<span style="font-size: 14px; opacity: 0.5; text-align:center; display: block"><i class="last-update-share">Последнее обновление: только что</i></span><canvas class="my-4" id="postings_per_hour" width="100" height="100"></canvas>');
          var ctx5 = document.getElementById('postings_per_hour');
          var myChart = new Chart(ctx5, {
            type: 'pie',
            data: {
  ...
  options: {
    ...
    onClick: function (event, elements) {
      if (elements.length > 0) {
        ...
        var value = this.data.datasets[element.datasetIndex].data[element.index];

        // Здесь можно задать URL соответствующий точке.
        var url = `/admin/modules/ozon2/logs/stock_c.php?time=${label}`;
...
$(document).on('click', '#show-log', function(e){
  e.preventDefault();
  $.ajax({
    url: '/admin/modules/ozon2/ajax/get_stock_by_date.php',
    method: 'post',
