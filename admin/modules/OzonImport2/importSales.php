...
class OzonImportSalesClass{
	public function __construct(){
		...
		$strSql = "SELECT * FROM wdhs_ozon_fbo_price_new";
		$results = $this->db->Query($strSql, false, $err_mess.__LINE__);
		while ($row = $results->Fetch()){
		...
		$strSql = "SELECT * FROM wdhs_ozon_fbo_sebes_new";
		$results = $this->db->Query($strSql, false, $err_mess.__LINE__);
		while ($row = $results->Fetch()){
    ...
    while ( $row = $results->Fetch() ){
      $this->markups[$row['model']] = $row['model'];
		...
		$strSql = "SELECT * FROM wdhs_ozon_sales_info_new";
    $results = $this->db->Query($strSql, false, $err_mess.__LINE__);
    $this->markups = [];
    while ( $row = $results->Fetch() ){
	...
	public function UpdateTmpTable(){
		...
		$this->usesNow = array();
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/sales/log_3.txt", print_r('start', true));
		...
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/sales/detail/{$this->date}.txt", print_r('###', true).PHP_EOL, FILE_APPEND);
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/sales/detail/{$this->date}.txt", print_r(json_encode(array($this->time => $this->detail)), true).PHP_EOL, FILE_APPEND);

		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/sales/log.txt", print_r(json_encode($this->arLog), true));

		print_r('<br>');
		foreach ($this->salesActive as $key => $sale) {
			print_r('<b><a target="_blank" href="/admin/modules/ozon2/sales/log.php?ID='.$sale['sale_id'].'">Акция</b> - '.$sale['name'].' ('.$sale['sale_id'].')</a><br>');
  ...
  public function getItems(){
    ...
    $arSelect = Array("ID","IBLOCK_ID","IBLOCK_SECTION_ID","PROPERTY_OZON_ACTIVE","PROPERTY_CML2_ARTICLE","PROPERTY_PRICE_OZTI","PROPERTY_WBARTICLE","PROPERTY_TYPEOFSKLAD","PROPERTY_OZON_ID_TI","PROPERTY_BRAND");
    $arFilter = Array(
    ...
    while ($el = $result->GetNext()){
		...
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/sales/tmp/arUpdateSale.txt", print_r($this->items, true));
		// print_r($this->items);
		// die();
	}
	...
	public function getSebes(){
		foreach ($this->items as $key => &$v) {
			$tmpPrice = array();
			$strSql = "SELECT price, id, count, model, supplier_id FROM ci_price WHERE model = '".$v['MODEL']."' AND active_ozti = 'Y' ORDER BY price ASC" ;
			$results = $this->db->Query($strSql, false, $err_mess.__LINE__);
			while ($row = $results->Fetch()){
				$tmpPrice[$row['id']] = [
	...
	public function GetActiveSales() {
		...
		$strSql = "SELECT * FROM wdhs_ozon_sales_new WHERE STR_TO_DATE(date_start, '%d.%m.%Y') < STR_TO_DATE('".$dateStart."', '%d.%m.%Y')";
...
public function CheckItemsAll()
{
	...
	file_put_contents(
		"/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport2/logs/sales/log_2.txt",
...
*/
	...
	public function updateSales()	{
		...
		foreach ($res['result'] as $key => $value){
			if (isset($this->salesActiveNotSort[$value['id']])) {
				...
				$this->db->Update("wdhs_ozon_sales_new", $ins, "WHERE sale_id={$value['id']}", $err_mess.__LINE__);
	...
	public function updatePriceTable()	{
		...
		$this->db->Query("DELETE FROM wdhs_ozon_sales_prices_new WHERE 1=1", false, $err_mess.__LINE__);
		foreach ($this->forUpdatePrice as $value) {
			...
			$this->db->Insert("wdhs_ozon_sales_prices_new", $in, $err_mess.__LINE__);
		}

		$this->db->Query("DELETE FROM wdhs_ozon_sales_info_new WHERE 1=1", false, $err_mess.__LINE__);
		foreach ($this->info as $model => $sale) {
			foreach ($sale as $sale_id => $v) {
				...
				$this->db->Insert("wdhs_ozon_sales_info_new", $in, $err_mess.__LINE__);
			}
		}
	}
