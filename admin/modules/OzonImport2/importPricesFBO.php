...
class OzonImportPricesFBO{
    ...
    $this->token = $arSetting['key'];
		...
		//Агент-Инфо
		$sql = "UPDATE ozon_agents SET status_text = 'Получаем данные для выгрузки цен', percent = '60' WHERE code = 'checkFbo_{$this->cabinet}'";
	...
	public function run(){
		...
		//Агент-Инфо
		$sql = "UPDATE ozon_agents SET status_text = 'Подготовка массива для выгрузки цен', percent = '70' WHERE code = 'checkFbo_{$this->cabinet}'";
		$this->CurDB->query($sql);
    ...
    $this->BuildArray();
		...
		//Агент-Инфо
		$sql = "UPDATE ozon_agents SET status_text = 'Выгружаем цены', percent = '80' WHERE code = 'checkFbo_{$this->cabinet}'";
		$this->CurDB->query($sql);
  ...
  public function getItems(){
    ...
    $arFilter = Array(
      ...
      "=PROPERTY_CML2_ARTICLE" => $this->articles,
    ...
    while ($el = $result->GetNext()){
			if (isset($this->checkFBOPrice[$el['PROPERTY_CML2_ARTICLE_VALUE']]) && !isset($this->checkSalePrice[$el['PROPERTY_CML2_ARTICLE_VALUE']])) {
