...
class checkFBONEW{
  ...
  public function checkFBOStock()
  {
		if (!empty($this->stockFbo)) {
			$this->db->Query("DELETE FROM wdhs_ozon_fbo_stock_prev_new WHERE 1=1", false, $err_mess.__LINE__);
			...
			foreach ($this->checkFBOStock as $key => $value) {
				...
				$this->db->Insert("wdhs_ozon_fbo_stock_prev_new", $in, $err_mess.__LINE__);
			}
