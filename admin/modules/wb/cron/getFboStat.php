  ...
  public function getItems()
  {
      ...
      while ($el = $result->GetNext()){
        if (empty($el['PROPERTY_WBARTICLE_VALUE']) or $el['PROPERTY_WBARTICLE_VALUE'] == '') {
          ...
          $this->items[$el["PROPERTY_WBARTICLE_VALUE"]] = [
			...
			if ( !empty($this->items) ){
				$this->writeLog('Получены данные из бт ('.count($this->items).')');
			}else{
				$this->writeLog('Ошибка! Данные из бт не получены');
			}
    }
    ...
    public function getStockFbo()
    {
      $fromMS = $this->getTurnover();
			if ( !empty($fromMS) ){
				$this->writeLog('Получены данные о себестоимости из МС ('.count($fromMS).')');
			}else{
				$this->writeLog('Ошибка! Данные о себесе не полчены');
			}
      $this->stockFbo = array();
        ...
        if (!empty($res['result']['items'])) {
          foreach ($res['result']['items'] as $key => $value) {
			...
			if ( !empty($this->stockFbo) ){
				$this->writeLog('Получены данные об остатках ФБО ('.count($this->stockFbo).')');
			}else{
				$this->writeLog('Ошибка! Данные об остатках не получены');
			}
    }
