...
//    CIBlockElement::SetPropertyValuesEx($arFields["ID"], 16, array('MARKETPLACE_OZON_TAGS' => ''));
    CIBlockElement::SetPropertyValuesEx($arFields["ID"], 16, array('MARKETPLACE_WB_TAGS' => ''));
}
 ...
 * Заполнение тегов WB
 */
$strSqlWb = "SELECT * FROM `ci_configurator_tags` WHERE active = 'Y' AND resource = 'WB' AND properties_json IS NOT NULL AND properties_json <> '' ORDER BY `sort_order` DESC";
$tagsWb = $DB->Query($strSqlWb, false);

while ($row = $tagsWb->Fetch()):
    ...
    foreach($sections as $sectionId) {
        ...
        if (!empty($duplicateProperties)) {
            foreach ($duplicateProperties as $propertyCode => $propertyValues) {
                foreach ($propertyValues as $propertyValue) {
                    ...
                    while ($obDuplicate = $resDuplicate->GetNextElement()) {
                        ...
                        $db_props = CIBlockElement::GetProperty(16, $arFieldsDuplicate["ID"], array("sort" => "asc"), Array("CODE"=>"MARKETPLACE_WB_TAGS"));
                        ...
                        CIBlockElement::SetPropertyValuesEx($arFieldsDuplicate["ID"], 16, array('MARKETPLACE_WB_TAGS' => $tagsList));
                    }
                }
            }
        } else {
            ...
            while ($ob = $res->GetNextElement()) {
                ...
                $db_props = CIBlockElement::GetProperty(16, $arFields["ID"], array("sort" => "asc"), Array("CODE"=>"MARKETPLACE_WB_TAGS"));
                if($ar_props = $db_props->Fetch()) {
                    $currentTags = $ar_props["VALUE"];
                }
                ...
                // Устанавливаем новые теги
                CIBlockElement::SetPropertyValuesEx($arFields["ID"], 16, array('MARKETPLACE_WB_TAGS' => $tagsList));
            }
        }
    }
endwhile;
