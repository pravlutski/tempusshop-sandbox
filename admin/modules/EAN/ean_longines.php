...
class LonginesEAN
{
  ...
  private function getItems():void
  {
    ...
    while ( $row = $result->GetNextElement() ){
      ...
      $this->items[$item['PROPERTY_CML2_ARTICLE_VALUE'] ] = [
        ...
        'formatted' => self::formatArticle( $item['PROPERTY_CML2_ARTICLE_VALUE'] ),
  ...
  private function getCodes1():void
  {
    ...
    foreach ( $this->items as $key => $codes ){
      $url = "https://www.upcitemdb.com/query?s={$codes['formatted']}&type=2";
      $ch = curl_init();
  ...
  private function getCodes2():void
  {
    ...
    foreach ( $models as $key => $arModel ){
      ...
      $model = str_replace('.','-', mb_strtolower($arModel['formatted']) );
  ...
  private function getCodes3():void
  {
    $i = 0;
    $models = $this->filterGoods();
    if ( empty($models) ) return;
  ...
  private function buildExcel():void
  {
    ...
    $filename = 'EAN_LONGINES_FILTERED.xlsx';
  ...
  private static function formatArticle( string $article ):string {
    if ( stripos($article, '.') ) return $article;
    if ( strlen($article) != 9 ) return $article;

    $part1 = substr($article, 0, 2);
    $part2 = substr($article, 2, 3);
    $part3 = substr($article, 5, 1);
    $part4 = substr($article, 6, 2);
    $part5 = substr($article, 8, 1);
