...
class WBOrderMain {
  ...
  public function __construct( $cabinet = 'WR' )
  {
    $this->options = [
      ...
      'log_path' => $_SERVER["DOCUMENT_ROOT"] . '/admin/modules/WBImport/logs/orders/' . $cabinet . '/' . date('Y-m-d') . '_orders.txt',
  ...
  private function checkIfOrderExists( array $order ):bool
  {
    ...
    if ( $result->SelectedRowsCount() > 0 ){
      ...
      $arFilterOrder = array(
        'PROPERTY_VAL_BY_CODE_MAXYSS_WB_RID' => $order['rid'],
      );
      var_dump($arFilterOrder);
