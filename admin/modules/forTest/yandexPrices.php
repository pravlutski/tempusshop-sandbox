...
class YandexPriceReport
{
  ...
  public function checkReportAvailiabilty($report)
  {
    ...
    } while ( !empty( $resultDecoded['result']['status'] ) && $resultDecoded['result']['status'] == 'PROCESSING' );
  ...
  private function writeDB($data)
  {
    ...
    $offers = array_values( array_chunk($data, 500) );
    foreach ($offers as $key => $offerPack) {
      $strSql = "INSERT INTO ci_yandex_price ('name', 'bitrix_id', 'minPrice', 'minPrice2') ";
      foreach ($offerPack as $key => $offer) {
        if ( $key == count($data) - 1 ){
          $strSql .= "VALUES ('{$offer['article']}', '{$offer['sku']}', '{$offer['ourPrice']}', '{$offer['newPrice']}')";
        }else{
          $strSql .= "VALUES ('{$offer['article']}', '{$offer['sku']}', '{$offer['ourPrice']}', '{$offer['newPrice']}'),";
        }
      }
