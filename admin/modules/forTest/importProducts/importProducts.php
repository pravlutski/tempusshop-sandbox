...
class WBImportProducts
{
  ...
  private $itemsForUpdate = []; // Массив с товарами для обновления карточек
  private $itemsForUpload = []; // Массив с товарам для создания карточек
  ...
  private $noNmidCards = []; // Созданные карточки, для которых не указан nmid в системе
  ...
  function __construct( $cabinet = 'WR', $arModels = [], $customLogPath = false )
  {
    ...
    $this->logPath = '/var/www/bitrix/data/www/tempusshop.ru/admin/modules/WBImport/logs/products/products_cron_'. date('Y-m-d') .'.txt';
  ...
  // Мастер методы для вызова из других скриптов
  ...
  {
    $this->noNmidCards = [
      ['vendorCode' => 'W_CASIO_A-158WA-1', 'bitrix_id' => 13263],
      ['vendorCode' => 'W_CASIO_GA-100-1A1', 'bitrix_id' => 1002],
      ['vendorCode' => 'W_CASIO_BG-5601-1E', 'bitrix_id' => 2249],
      ['vendorCode' => 'W_CASIO_BG-6903-2E', 'bitrix_id' => 2282],
      ['vendorCode' => 'W_CASIO_BA-110-7A1', 'bitrix_id' => 3056],
      ['vendorCode' => 'W_CASIO_BGA-180-7B2', 'bitrix_id' => 4133],
      ['vendorCode' => 'W_CASIO_BA-110BC-1A', 'bitrix_id' => 4227],
     ];
    $this->getCreatedNmids();
    $this->saveCreatedNmids();
  }
  ...
  public function getItems():void
  {
    ...
    $arSelect = ['ID', 'IBLOCK_ID', 'DETAIL_PICTURE', 'PROPERTY_WBARTICLE2','PROPERTY_WBARTICLE3'];
    ...
    while ( $row = $result->GetNext() ){
      ...
      if ( empty($card['NMID']) || empty($card['CHRTID']) ){
  ...
  private function buildArrayMedia( bool $useInfograph ):void
  {
    ...
    $this->writeLog( 'Got media for '. count($this->requestBody) . ' card(s)' );
    var_dump($this->requestBody);
  }
  ...
  private function updateMedia():void
  {
    ...
    foreach ( $this->requestBody as $data ){
      ...
      if ( $res['http_code'] != 200 || empty( $res ) ){
        $this->writeLog( 'WB returned an error: ' . PHP_EOL . $res );
        continue;
      }
      $counter++;
    }
    $this->writeLog( "Media for {$counter} cards was updated" );
  }
  ...
  private function upload():void
  {
    ...
    if ( count($this->requestBody) > 100 ){
      ...
      foreach ($arData as $key => $chunk) {
        ...
        $this->writeLog( "Success! Group {$key} had to be uploaded. Code: " . $res['http_code'] );
      }
    }else{
      $res = $this->request(
        'https://content-api.wildberries.ru/content/v2/cards/upload',
      ...
      if ( $res['http_code'] != 200 ){
        $this->writeLog( "Error! Goods was not uploaded: " . print_r($res, 1) );
        return;
      }
      $this->writeLog( "Success! Goods had to be uploaded. Code: " . $res['http_code'] );
    }
  }
  ...
  private function update():void
  {
    ...
    if ( count($this->requestBody) > 3000 ){
      ...
      foreach ( $arData as $key => $chunk ){
        ...
        $this->writeLog( "Success! Group {$key} had to be updated. Code: " . $res['http_code'] );
      }
    }else{
      var_dump( json_encode($this->requestBody) );
      $res = $this->request(
        'https://content-api.wildberries.ru/content/v2/cards/update',
        $this->header,
      ...
      if ( $res['http_code'] != 200 ){
        $this->writeLog( "Error! Goods was not updated: " . print_r($res, 1) );
        return;
      }
      $this->writeLog( "Success! Goods had to be updated. Code: " . $res['http_code'] );
    }
  }
  ...
  private function saveCreatedNmids():void
  {
    ...
    $this->writeLog( count($this->recievedNmids) . " will be saved" );
 ...
 $ar = ['A-158WA-1','BG-5601-1E', 'BG-6903-2E', 'BA-110-7A1', 'BGA-180-7B2', 'BA-110BC-1A'];
 $ar = ['AW-90H-9E'];
( new WBImportProducts( 'WR', $ar, false ) )->run();
