...
class OzonOrderMain {
  ...
  {
    // Озон в кабинете выводит время по москве, а сам работает по гринвичу, поэтому минус три часа
    $data = [
      'filter' => [
        'since' => date( 'Y-m-d', strtotime('- 1 day') ) . 'T21:00:00Z',
  ...
  public function processOrders():void
  {
    ...
    foreach ( $this->ordersApi as &$orderApi ){
      // Тест
      $orderApi['status'] = 'awaiting_packaging';
      $orderApi['customer'] = [
        'name' => 'Жмышенко Валерий Альбретович',
        'address_tail' => 'Финляндия, Хельсинки, Пролетарская 15 кв 88',
        'country' => 'Финляндия',
        'region' => 'Финляндия',
        'pvz_code' => '1723',
        'city' => 'Хельсинки',
        'comment' => 'Все в порядке. Это тестовый заказ',
        'posting_number' => $orderApi['posting_number']
      ];
      if ( $this->checkIfOrderExists($orderApi) ){
        ...
        if ( $orderApi['status'] != 'awaiting_packaging' ){
    ...
    // Блок, где заказ создается в битриксе
        ...
        die('Попал в апдейт');
        $this->writeLog( 'Order ' . $orderApi . ' already exists. Trying to update status');
  ...
  private function updateOrderStatus( array $order ):void
  {
    ...
    if ( $statusBX = $this->mapStatus($order['status']) ){
      ...
      $strSql = "SELECT status FROM {$this->options['order_table_name']} WHERE posting_number = '{$order['posting_number']}'";
      $result = $this->db->Query($strSql, false, $err_mess.__LINE__);
      $statusCurrent = $result->Fetch()['status'] ?? '';

      if ( !empty($statusCurrent) && $statusCurrent != $order['status'] ){
        $strSql = "SELECT order_bid FROM {$this->options['order_table_name']} WHERE posting_number = '{$order['posting_number']}'";
        $result = $this->db->Query($strSql, false, $err_mess.__LINE__);
        $orderBID = $result->Fetch()['order_bid'] ?? '';
        $orderBX = Order::load( $orderBID );
  ...
  public static function convertVatRate(int $vat_id = 1):float // Скопипиздил метод из старого модуля
  {
      $result = 0.00;
