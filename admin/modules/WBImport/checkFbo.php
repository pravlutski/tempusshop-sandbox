...
class checkFBO{
  ...
  public function checkFBOStock()
  {
	  ...
	  $this->db->Query("DELETE FROM wdhs_wb_fbo_stock WHERE 1=1", false, $err_mess.__LINE__);
		$this->db->Query("DELETE FROM wdhs_wb_fbo_sebes WHERE 1=1", false, $err_mess.__LINE__);
		//print_r($this->items);
		foreach ($this->stockFbo as $key => $value) {
	...
	public function PrintResult()
	{
		...
		// 	print_r($value);
		// }
		file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/modules/WBImport/logs/fbo/log.txt", print_r(json_encode($this->answer), true).PHP_EOL);
	}
