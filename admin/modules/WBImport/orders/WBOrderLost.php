...
class WBOrderMain {
  ...
  public function __construct( $cabinet = 'WR' )
  {
    $this->options = [
      ...
      // 'log_path' => $_SERVER["DOCUMENT_ROOT"] . '/admin/modules/WBImport/logs/orders/' . $cabinet . '/' . date('Y-m-d') . '_orders_lost.txt',
  ...
  public function loadOrdersWB():void
  {
    ...
    $limit = 1000;
    $next = 0;
    $dateFrom = strtotime(date('Y-m-d') . ' 00:00:00' . '- 1 day');
    $dateTo = time();
