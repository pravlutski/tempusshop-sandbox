...
$tmp = file_get_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/modules/OzonImport/logs/stocks/control/'.$curdate.'.txt');
$arTmp = explode('#SPLIT#',$tmp);
foreach ($arTmp as $key => $value) {
  $arSoloLog = json_decode($value,true);
  //print_r($arSoloLog);
  ...
  options: {
    ...
    onClick: function (event, elements) {
      if (elements.length > 0) {
        ...
        var url = `/admin/modules/ozon/logs/stock_c.php?time=${label}`;
