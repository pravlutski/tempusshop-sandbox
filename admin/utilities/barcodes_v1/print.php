...
$isGroupOrder = $_REQUEST["is_group_order"] ?? false;
$controlMark = $_REQUEST["control_mark"] ?? '';

$logger->log("LOG", "Запрос", ['userID' => $userID, '_REQUEST' => $_REQUEST]);

function splitByWords($line, $maxLength = 50) {
    $result = [];
...
if ($source == 'wb') {
	...
	if ($typeScan && in_array($typeScan, ['manual', 'scanner'])) {
		OrderPrintManager::addPrintRecord($orderID, $userID, $typeScan, $productID, $numberID, $controlMark);
	}
	//Close and output PDF document
	$pdf->Output();
} elseif($source == 'ozon') {
	$arOrder = explode(",", $_REQUEST["order_market_number"]);
	$arOrderPrint = [];//prent($arOrder);die;
	if(!$arOrder) die("нет кодов");
	...
	if ($returnCode === 0) {
		if ($typeScan && in_array($typeScan, ['manual', 'scanner'])) {
			...
			if ($productID && $numberID) {
				...
				// лезем в заказ и проставляем на все позиции
				
				$order = $objService->getOrder([], ['ID' => $orderID])[0];
					...
					foreach($order["BASKET"] as $k => $arItem){
						...
						if ($is_group) {
							for($i = 0; $i < $arItem["QUANTITY"]; $i++){
								$number_id = $i + 1;
								OrderPrintManager::addPrintRecord($orderID, $userID, $typeScan, $arItem["PRODUCT_ID"], $number_id, $controlMark);
							}
						}
	...
	$outputPath = $_SERVER['DOCUMENT_ROOT'] . "/upload/stickers/sdek/outputPath.pdf";

	$fileSticker = $_SERVER['DOCUMENT_ROOT'] . "/upload/stickers/sdek/{$order_market_number}.pdf";

	if (file_exists($fileSticker)) {
		if ($productID && $numberID || !$isGroupOrder) {
			OrderPrintManager::addPrintRecord($orderID, $userID, $typeScan, $productID, $numberID, $controlMark);
		} else {
			// лезем в заказ и проставляем на все позиции
		...
		$fileContent = file_get_contents($fileSticker);
		$filename = "sticker_sdek_{$order_market_number}.pdf";
		header('Content-Type: application/pdf');
		header('Content-Length: '.strlen( $fileContent ));
		header('Content-disposition: inline; filename="' . $filename . '"');
		header('Cache-Control: public, must-revalidate, max-age=0');
	...
	$barcode = trim(htmlspecialchars($_REQUEST["order_market_number"]));
	$order = $objService->getOrder([], ['ID' => $orderID])[0];
	...
	$commentLine = explode("\n", $order['COMMENTS']);
	//prent($commentLine);die;
	$filename = $_SERVER['DOCUMENT_ROOT'] . "/upload/barcodes/avito/{$barcode}.png";

	try {

		$pdf = new TCPDF('L', 'mm', array(40, 58), true, 'UTF-8', false);
		$pdf->setPrintHeader(false);
		$pdf->setPrintFooter(false);
		$pdf->SetMargins(0, 0, 0, 0);
		$pdf->SetAutoPageBreak(false, 0);
		...
		foreach ($commentLine as $line) {
			if (strlen($line) > 40) {
				$splitLine = splitByWords($line, 40);
				foreach ($splitLine as $_line) {
					$pdf->SetXY(2, $startY);
					$pdf->Cell(40, 4, $_line, 0, 1, 'L');
					$startY += 3;
				}
			} else {
				$pdf->SetXY(2, $startY);
				$pdf->Cell(40, 4, $line, 0, 1, 'L');
				$startY += 3;
			}
		}
		...
		ob_clean();
		$pdf->Output("barcode_avito_{$barcode}.pdf", 'I');
