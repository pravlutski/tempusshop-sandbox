...
if($_REQUEST["submit_profile"] && $profile_name){
...
<form action="/admin/utilities/set_prop_json/" method="post" id="form_set_prop" >
	<div class="profile-block">
		...
		<select class="form-control select_w" name="set_profile" id="set_profile">
				...
				<?foreach($arValues as $value):?>
					...
					<?foreach($sel_prop as $s_prop_code => $values):?>
						...
						<tr>
							...
							<td class="td-prop">
								<select class="form-control select_w" name="prop" data-prop="<?=$code?>" data-value="<?=$value?>" onchange="getVariants(this);">
									<option value="">--- Выберите сойство ---</option>
									<?foreach($arResult["PROPS"] as $key => $arItem):?>
...
function getVariants( obj ){
	...
	var prop_options = '';
			...
			{
				$(td_variants).html(
					'<input type="text" name="PROPS[' + f_prop + '][' + f_value + '][' + PROPS[code].id + '][VALUES]" class="form-control">'
				);
			}
}
