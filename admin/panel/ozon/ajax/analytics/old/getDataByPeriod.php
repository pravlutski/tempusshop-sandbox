...
$sellerInfo = [];
$strSqlC = "SELECT * FROM ozon_competitors WHERE seller = 'TEMPUS - Наручные часы'";
$resultC = $dbPanel->Query( $strSqlC );
$rowsC = $dbPanel->fetchAll( $resultC );
foreach( $rowsC as $row ){
  $sellerInfo[ $row['date'] ][ $row['model'] ] = $row['g_price'];
}
...
function colorCell($percent, $column){
  ...
  foreach ($steps as $value) {
    if ( $percent > floatval($value) ){
      $className = 'green-' . $value;
      return $className;
    }
  }
  foreach ( $steps as $value ){
    if ( $percent < floatval($value / -1) ){
      $className = 'red-' . $value;
      return $className;
    }
  }
...
foreach ( $tops as $model ){
  if ( isset($tableDataTmp[$model]) ){
    $tableData[$model] = $tableDataTmp[$model];
  }
}
...
<div class="table-main">
 <table class="table table-striped table-data">
   ...
   <tbody>
      ...
      foreach( $modelRow as $colName => $dbRow ){
        ...
        foreach ( $dbRow as $date => $data ){
            ...
            if ( $colName != 'model'  ){
              if ( $colName == 'orders_count' && $date == date('Y-m-d') ) continue;
              if ( in_array($colName, $arDiff) ){
                // echo '<td style="max-width: 105px !ismportant;min-width: 105px !important;">';
                echo ( $colName == 'g_price' && $data[$colName] == 0) ? '<td style="max-width: 105px !important;min-width: 105px !important;" class="red-10">' : '<td style="max-width: 105px !important;min-width: 105px !important;">';
                echo ( ($colName == 'g_price' || $colName == 'g_price' ) && $data[$colName] == 0) ? 'Н/Д' : $data[$colName];
                echo '</td>';
                $diff = calculateDifference($arCalc, $model, $date, $colName);

                if ( $diff != NULL ){
                  if ( $last_date == $date ){
                    echo '<td class="bordered '.$diff['className'].'" style="max-width: 105px !important;min-width: 105px !important;">';
                  }else{
                    echo '<td class="'.$diff['className'].'" style="max-width: 105px !important;min-width: 105px !important;">';
                  }
                  if ( $diff['value'] >= 1 ){
                    $total_inc[$i + $k] += 1;
                  ...
                  echo '</td>';
                  if ( $date == $last_date ){
                    $columnSpace[$i + $k] = $colName;
                    echo "<td class='bordered' style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'></td>";
                  }
                }
              }else{
                if ( $colName == 'sale_name' ){
                  ...
                  $class = ($colName == 'sale_name' && $data[$colName] == 'Не участвует') ? 'red-10' : 'green-10';

                  echo ($date == $last_date) ? '<td class="bordered '.$class.'" '.$title.' style="max-width: 105px !important;min-width: 105px !important;">' : '<td '.$title.' class="'.$class.'" style="max-width: 105px !important;min-width: 105px !important;">';
                }else{
                  ...
                  echo "<td class='bordered ".$class."' style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'>";
                }
     ...
     // print_r( $total );
     // file_put_contents( '/var/www/bitrix/data/www/tempusshop.ru/admin/panel/ozon/ajax/analytics/log.txt', print_r( $columnSpace, 1) );
     ?>
   </tbody>
 </table>
</div>
       ...
       foreach ( $total_inc as $key => $pos ){
         ...
         if ( $pos == 0 && $total_dec[$key] == 0){
           if ( $columnSpace[$key] && $key != array_key_last($total_inc) ){
             echo "<td class='bordered' style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'></td>";
             echo "<td style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'><span class='row-header'>{$rowHeadersPositive[$columnSpace[$key]]}</span></td>";
           }else{
             echo "<td style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'></td>";
           }
         }else{
           if ( $columnSpace[$key] && $key != array_key_last($total_inc) ){
             echo "<td class='bordered' style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'>{$pos}</td>";
             echo "<td style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'><span class='row-header'>{$rowHeadersPositive[$columnSpace[$key]]}</span></td>";
           }else{
             echo "<td style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'>{$pos}</td>";
           }
         }
       }
       ?>
     ...
     <tr>
       ...
       foreach ( $total_dec as $key => $pos ){
         ...
         if ( $pos == 0 && $total_dec[$key] == 0){
           if ( $columnSpace[$key] && $key != array_key_last($total_inc) ){
             echo "<td class='bordered' style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'></td>";
             echo "<td style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'><span class='row-header'>{$rowHeadersNegative[$columnSpace[$key]]}</span></td>";
           }else{
             echo "<td style='padding:0.5rem;max-width: 105px !important;min-width: 105px !important;'></td>";
           }
         }else{
