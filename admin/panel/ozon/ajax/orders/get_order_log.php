...
$logPathMain = "{$_SERVER['DOCUMENT_ROOT']}/admin/panel/engine/ozon/orders/logs/IP/{$_POST['date']}_orders.txt";
$logPathStatus = "{$_SERVER['DOCUMENT_ROOT']}/admin/panel/engine/ozon/orders/logs/IP/{$_POST['date']}_order_update.txt";
...
if ( !empty($logTextStatus) && $mode == 'short' ){
  $arText = explode(PHP_EOL, $logTextStatus);
  $logTextStatus = '';
  foreach ( $arText as $row ){
    if ( !strrpos($row, 'is completed') ){
