...
if (!empty($res['result'])) {?>
  ...
  <?} else {?>
    <?php foreach ($arSale as $id => $v): ?>
      ...
      <tr id="<?=$v['id']?>"  class="<?=$v['class']?>">
        <th scope="row" ><input  style="width:50px;" name="data[<?=$v['id']?>][sort]" value="<?=$salesActive[$v['id']]['sort']?>" /></th>
        <td><input hidden name="data[<?=$v['id']?>][active]" value="<?=$v['active']?>" />
          <span>
            <?if ($v['active'] == "1"){echo "ДА";}else{echo "НЕТ";}?>
          </span>
        </td>
        <td><input hidden name="data[<?=$v['id']?>][name]" value="<?=$v['name']?>" /><span><?=$v['name']?> (OLD)</span></td>
        <td><input hidden name="data[<?=$v['id']?>][date_start]" value="<?=$v['date_start']?>" /><span><?=$v['date_start']?></span></td>
        <td><input hidden name="data[<?=$v['id']?>][date_end]" value="<?=$v['date_end']?>" /><span><?=$v['date_end']?></span></td>
        <td><input style="width:50px;" name="data[<?=$v['id']?>][perc]" value="<?=$salesActive[$v['id']]['perc']?>" /></td>
        <td><input style="width:50px;" name="data[<?=$v['id']?>][skd]" value="<?=$salesActive[$v['id']]['skd']?>" /></td>
        <td><input style="width:50px;" name="data[<?=$v['id']?>][skd_fbo]" value="<?=$salesActive[$v['id']]['skd_fbo']?>" /></td>
        <td><?=$v['merg']?></td>
        <td><input hidden name="data[<?=$v['id']?>][potencial]" value="<?=$v['potencial']?>" /><span><?=$v['potencial']?></span></td>
        <td><input hidden name="data[<?=$v['id']?>][top_models]" value="<?=$v['top_models']?>" /><span style="width:50px"><?=$v['top_models']?></span></td>
        <td><input hidden name="data[<?=$v['id']?>][uses]" value="<?=$v['uses']?>" /><span><?=$v['uses']?></span></td>
      ...
      <?} else {?>
        <?if (trim($salesActive[$v['id']]['name']) != $v['name']) {?>
          <?//print_r($salesActive[$v['id']]);?>
          <?//print_r($v['id']);?>
          <tr id="<?=$v['id']?>"  class="<?=$salesActive[$v['id']]['class']?>">
            <th scope="row" ><input  style="width:50px;" name="data[<?=$v['id']?>][sort]" value="<?=$salesActive[$v['id']]['sort']?>" /></th>
            <td><input hidden name="data[<?=$v['id']?>][active]" value="<?=$v['active']?>" />
              <span>
                <?if ($v['active'] == "1"){echo "ДА";}else{echo "НЕТ";}?>
            ...
            <td><input hidden name="data[<?=$v['id']?>][name]" value="<?=$v['name']?>" /><span><?=$v['name']?> (NEW2)</span></td>
            <td><input hidden name="data[<?=$v['id']?>][date_start]" value="<?=$v['date_start']?>" /><span><?=$v['date_start']?></span></td>
            <td><input hidden name="data[<?=$v['id']?>][date_end]" value="<?=$v['date_end']?>" /><span><?=$v['date_end']?></span></td>
            <td><input style="width:50px;" name="data[<?=$v['id']?>][perc]" value="<?=$salesActive[$v['id']]['perc']?>" /></td>
            <td><input style="width:50px;" name="data[<?=$v['id']?>][skd]" value="<?=$salesActive[$v['id']]['skd']?>" /></td>
        ...
        <?} else {?>
          <tr id="<?=$salesActive[$v['id']]['id']?>"  class="<?=$salesActive[$v['id']]['class']?>">
            ...
            <td><input hidden name="data[<?=$salesActive[$v['id']]['id']?>][name]" value="<?=$salesActive[$v['id']]['name']?>" /><span>(NEW3) <?=$salesActive[$v['id']]['name']?></span></td>
