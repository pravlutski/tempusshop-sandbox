...
$action_id = $_GET['ID'];
$tmp = file_get_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/panel/engine/ozon/logs/'.$cabinet.'/sales/log.txt');
...
?>
<div class="row">
  <form method="GET" action="" class="s-form" style="display: flex;
  gap: 10px;">
    <div class="input-group" style="max-width: 300px;  gap: 10px;  align-items: center;">
      <label>Кабинет</label>
  ...
  padding-bottom: 20px;">
      <div class="header_table">
          <div class="title_header_table">
            ...
            <span><a style="color:#111;text-decoration:none;" href="?ID=<?=$key?>&cabinet=<?=$cabinet?>"><?=$salesActive[$key]['name']?></a></span>
          </div>
      </div>
  </div>
  <?}?>
...
<?php foreach ($log as $key => $value): ?>
  ...
  switch ($key) {
    case 'STAY':
        $name = "ИДЕАЛЬНЫЕ УСЛОВИЯ. Остались в акции без изменний";
        $bg = "background-color: rgba(212, 242, 64, 0.44);";
        break;
    case 'STAY-UP':
        $name = "Остальсь в акции. Увеличили цену в акции";
        $bg = "background-color: rgba(212, 242, 64, 0.44);";
        break;
    case 'NOT_ADD':
        $name = "Не добавлены";
