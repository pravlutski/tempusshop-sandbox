...
$status = [
  'not-add' => '#f4b9b9',
  'delete' => '#f4b9b9',
  'not-add-prioretet' => '#f4b9b9',
  'delete-prioretet' => '#f4b9b9',
  'stay' => '#a0ddf0',
  'stay-with-up' => '#a0ddf0',
...
if (isset($_POST['model']) && !empty($_POST['model'])) {
  ...
  if (isset($_POST['sale_id']) && $_POST['sale_id'] != '0' && $_POST['sale_id'] != '') {
    $sale = $_POST['sale_id'];
    $result = $CurDB->query("SELECT * FROM ozon_sales_log_{$cabinet} WHERE model='{$model}' AND sale_id = {$sale}");
  } else {
    $result = $CurDB->query("SELECT * FROM ozon_sales_log_{$cabinet} WHERE model='{$model}'");
  }
  $rows = $CurDB->fetchAll($result);
  foreach ($rows as $row) {
...
foreach ($arResult as $k => $v) {
  ?>
  <?//print_r($v);?>
  <?if ($v['data']['status'] == 'not-add-prioretet') {?>
    <div class="col-sm-12" style="background-color: <?=$status[$v['data']['status']]?>;margin-top: 15px;padding-top: 20px;    padding-bottom: 20px;">
        <div class="header_table">
            <div class="title_header_table">
               <b><?=$v['datetime']?></b> - <?=$v['data']['sale']?><br>
               <b>Статус:</b> <i><?=$statusDesc[$v['data']['status']]?></i><br><br>
                  <p style="font-size:14px;"><b>Товар не добавлен в акцию, т.к. вошел в другую акцию по приоритету</b><br>

            </div>
        </div>
    </div>
  <?} else if ($v['data']['status'] == 'delete-prioretet') {?>
    <div class="col-sm-12" style="background-color: <?=$status[$v['data']['status']]?>;margin-top: 15px;padding-top: 20px;    padding-bottom: 20px;">
        <div class="header_table">
            <div class="title_header_table">
               <b><?=$v['datetime']?></b> - <?=$v['data']['sale']?><br>
               <b>Статус:</b> <i><?=$statusDesc[$v['data']['status']]?></i><br><br>
                  <p style="font-size:14px;"><b>Товар удален из акции, т.к. вошел в другую акцию по приоритету</b><br>
          ...
          <div class="title_header_table">
             ...
             <b>Статус:</b> <i><?=$statusDesc[$v['data']['status']]?></i><br><br>
                ...
                <div class="price-analiz">
                  ...
                  <div class="needed">
                    <b>Необходимо:</b><br>
                    <?=$v['data']['ozon_answer']['max_action_price']?><br>
                    <?=$v['data']['cur_min_prof']?>
                  </div>
                  <div class="raschet">
                    <b>Расчет:</b><br>
...
if (isset($_POST['model']) && !empty($tmp) && (empty($_POST['model']))) {
  ?>
  <div class="col-sm-12" style="background-color: rgba(221, 111, 77, 0.42);margin-top: 15px;padding-top: 20px;
  padding-bottom: 20px;">
      <div class="header_table">
