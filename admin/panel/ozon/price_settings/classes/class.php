...
class SettingsManager
{
  ...
  private function calcualteGoalForPeriodPerModel( int $goal, int|bool $lastHour, int $nextHour ):int|bool
  {
    ...
    $sumT = 0;
    for ( $i = 0; $i <= $nextHour; $i++ ){
      $sumT += $goal * $coefs[$i];
    }

    if ( $lastHour > $currentHour ){
      $sumY = 0;
      for ( $i = $lastHour; $i < 24; $i++){
        $sumY += $goal * $coefs[$i];
      }
  ...
  private function calculateGoalsPeriod( array $items ):array
  {
    ...
    foreach ( $items as $item ){
      ...
      $lastRunHour = date( 'G', strtotime($item['intervals']['lastRunDate']) );
      $nextRunHour = date( 'G', strtotime($item['intervals']['nextRunDate']) );
      $result[ $item['model'] ] = $this->calcualteGoalForPeriodPerModel( $item['goal'], $lastRunHour, $nextRunHour );
    }
    return $result;
  }
