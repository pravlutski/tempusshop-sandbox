...
if ( $dateFrom == $dateTo ) $dateFrom = date('Y-m-d', strtotime('-4 day'));
$arPeriod = [];
$ds = $dateFrom;
while ( $ds <= $dateTo ){
  $arPeriod[] = $ds;
...
function getColorSppCell( string $model, string $date, array $arData ):string
{
  $result = '';
    ...
    if ( count($uniqueValues) >= 2 ) $result = 'warning-cell';
...
{
  $rows = $db->select( ['*'], 'wb_spp_analytics_by_hour' )->make();
  $result = [];
  ...
  foreach ( $rows as $key => $row ){
    ...
    foreach ( $keys as $k){
      ...
      $tableDataTmp[$row['model']][$k][$row['date']] = [
        ...
        'c_spp' => $c_spp == 100 ? '<span class="no-comps">Нет данных</span>' : $c_spp,
    ...
    foreach ( $arPeriod as $cd ){
      if ( !isset($daRow[$cd]) ){
        $tableDataTmp1[$model][$colName][$cd] = [
          ...
          'c_spp' => '<span class="no-comps">Нет данных</span>',
          'is_fbo' => $stockFbo[$cd][$model] ?? 0,
          'date' => $cd,
        ];
		$headerData[$colName][$cd] = ['date' => $cd, 'model' => $model];
      }else{
            ...
            if ( $colName != 'model' && $colName != 'nmid'  ){
              if ( in_array($colName, $arDiff) ){
                ...
                if ( $data[$colName] == 0 ){
                  ...
                  echo  ( $colName == 'our_price' ) ? 'Нет данных' : 'Нет в наличии';
                  echo '</td>';
                }else{
                  $classPrice = '';
                  if ( in_array($colName,['our_price', 'sell_price' , 'black_price']) ){
                    $classPrice = 'price-btn';
                    $data_attrs_p = "data-nmid='{$data['nmid']}' data-date='{$date}' data-model='{$model}'";
                  }
                  echo '<td class="'.$diff['className'].' '.$classPrice.'" style="max-width: 155px !important;min-width: 155px !important;" class="bordered" title="'.$diff['value'].'">';
                  if ( $diff == 0 ){
                    echo $data[$colName];
                  }else{
                ...
                elseif ( $colName == 'c_spp' || $colName == 'spp' ){
                  $class = 'spp-btn ';
                  $data_attrs = "data-nmid='{$data['nmid']}' data-date='{$date}' data-model='{$model}'";
                ...
                echo "<td class='bordered {$class}' {$data_attrs} style='padding:0.5rem;max-width: 155px !important;min-width: 155px !important;'>";
                echo $data[$colName];
                echo '</td>';
              ...
              echo '<a href="'.$link.'" target="_blank">' . $data[$colName] . '</a>';
              echo '</td>';
            }
            $i++;
            $c++;
