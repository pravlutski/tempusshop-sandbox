...
<div id="container">
  ...
  <div id="logs-block" class="order-logs tab" style="display:none">
    ...
    <div class="log-contol-block">
      ...
      <select id="cabinet" class="log-date-picker form-select" name="">
...
const ajax_folder_path = "/admin/panel/wb/ajax/orders/";

  $.ajax({
    url: ajax_folder_path + 'get_order_log.php',
    method: 'POST',
  ...
  $(document).on('click', '#save-orders',function(e){
    e.preventDefault();
    $.ajax({
      url: ajax_folder_path + 'save_order_status.php',
      method: 'POST',
  ...
  $(document).on('click', '#export-report',function(e){
    ...
    $('.download-btn').remove();
    $.ajax({
      url: ajax_folder_path + 'make_orders_report.php',
      method: 'POST',
