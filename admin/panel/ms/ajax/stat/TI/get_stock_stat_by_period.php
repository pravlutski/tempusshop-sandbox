...
$minDate = empty($_POST['min_date_stat']) ? date('Y-m-d', strtotime('-55 day')) : $_POST['min_date_stat'];
$maxDate = $_POST['max_date_stat'];

$strSql = "SELECT avg(stock) as stock, avg(price) as price FROM (SELECT date, sum(stock) as stock, sum(stock * price) as price FROM ozon_stock_fbo_stat_ti";
