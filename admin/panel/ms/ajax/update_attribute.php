...
if (!empty($arTypes)) {
  $DB->Query("DELETE FROM wdhs_ozon_attribute_new WHERE 1=1", false, $err_mess.__LINE__);
  foreach ($arTypes as $key => $value) {
    ...
    foreach ($dataAtt['result'] as $att) {
      if ($att['dictionary_id'] != '0') {
        $DB->Query("DELETE FROM wdhs_ozon_attribute_list_new WHERE attribute_id = '".intval($att['id'])."'", false, $err_mess.__LINE__);
        $next = 1;
        $lv_id = 0;
        while ($next == 1){
          $dataDic = array(
          ...
          foreach ($resD['result'] as $n => $p) {
            ...
            $DB->Insert("wdhs_ozon_attribute_list_new", $in, $err_mess.__LINE__);
          }
      ...
      //print_r($in);
      $DB->Insert("wdhs_ozon_attribute_new", $in, $err_mess.__LINE__);
    }
