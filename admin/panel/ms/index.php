...
<?$curdate = date('Y-m-d');?>
<?$APPLICATION->SetTitle('Главная - модуль Мой Склад');?>
<?$APPLICATION->SetPageProperty("page_h1", "Панель управления 'Мой Склад'");?>
<?
opcache_reset();
global $DB;
global $USER;
...
$CurDB = new DBPanel();
$result = $CurDB->query("SELECT * FROM ms_agents");
...
<div class="position-fixed bottom-0 end-0 p-3" style="z-index: 11">
  <div id="coplete_toast" class="toast hide align-items-center bg-warning" data-bs-delay="4000" role="alert" aria-live="assertive" aria-atomic="true">
    <div class="toast-header">
      ...
      <strong class="me-auto">Tempus MS Module</strong>
...
$(document).ready(function() {
    function attachEventHandlers() {
        $('#table-panel').on('click', '.kill-script', function() {
            ...
            $.ajax({
                url: '/admin/panel/ms/ajax/tech/proc_kill.php',
                method: 'POST',
                data: {
                    script: dataScript,
                    code: dataCode,
        ...
        $('#table-panel').on('click', '.run-script', function() {
            ...
            var dataName = $(this).data('name');
            $.ajax({
                url: '/admin/panel/ms/ajax/tech/proc_run.php',
                method: 'POST',
                data: {
                    script: dataScript,
                    code: dataCode,
        ...
        $('#table-panel').on('click', '.reboot-script', function() {
            ...
            var dataName = $(this).data('name');
            $.ajax({
                url: '/admin/panel/ms/ajax/tech/proc_reboot.php',
                method: 'POST',
                data: {
                    script: dataScript,
                    code: dataCode,
    ...
    function updatePanel() {
        $.ajax({
            url: '/admin/panel/ms/ajax/tech/update_panel.php',
            method: 'GET',
            success: function(response) {
    ...
    // Привязываем обработчики событий к родительскому элементу
    attachEventHandlers();
