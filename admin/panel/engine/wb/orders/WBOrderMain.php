...
class WBOrderMain {
  ...
  private $dictionary = []; // Словарь типа nmid => bitrix_id
  ...
  public function __construct( $cabinet = 'WR' )
  {
    ...
    $this->options = [
      ...
      'log_path' => $_SERVER["DOCUMENT_ROOT"] . '/admin/panel/engine/wb/logs/orders/' . $cabinet . '/' . date('Y-m-d') . '_orders.txt',
      // 'log_path' => $_SERVER["DOCUMENT_ROOT"] . '/admin/modules/forTest/' . date('Y-m-d') . '_orders.txt'
    ];
    switch ($cabinet) {
      case 'WR':
        $this->options['user_id'] = 135989;
        $this->options['price_set_filter'] = 'WB';
        break;
      case 'TL':
        $this->options['user_id'] = 161898;
        $this->options['price_set_filter'] = 'WBTL';
        break;
      case 'WT':
        $this->options['user_id'] = 191551;
        $this->options['site_id'] = 's2';
        $this->options['currency_code'] = 'BYN';
        $this->options['price_set_filter'] = 'BY';
        break;
      default:
        die("Invalid cabinet\n");
        break;
  ...
  public function loadOrdersWB():void
  {
    ...
    foreach( $res['orders'] as $field ){
      ...
      $orderItem = $this->getItemByNmid( $field['nmId'] );
      if ( empty($orderItem) ){
      ...
      $orderDataTmp['products'][] = [
        ...
        'price' => $this->getConvertedPrice( intval($field['currencyCode']), $field['salePrice'] ),
        'finishedPrice' => $field['convertedPrice'] / 100,
      ];
  ...
  public function processOrders():void
  {
    ...
    foreach ( $this->ordersApi as &$orderApi ){
      ...
      if ( $result->isSuccess() ){
        ...
        if ( $this->cabinet == 'WR' ){
          $tpId = 6;
        } elseif( $this->cabinet == 'WT' ) {
          $tpId = 19;
        } elseif ( $this->cabinet == 'TL' ){
          $tpId = 21;
        }
        ...
        // file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/panel/engine/wb/orders/logs.txt', print_r($resss,true) . PHP_EOL, FILE_APPEND );
        // file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/panel/engine/wb/orders/errors.txt', print_r($res->getErrorMessages(),true) . PHP_EOL, FILE_APPEND );
  ...
  private function writeInDatabase( array $order ):void
  {
    ...
    foreach ( $orderProducts as $product ){
      ...
      $productData = $this->getProductCostAndSupplier( $product['vendor_code'], $product['quantity'] );
  ...
  private function getProductCostAndSupplier( string $vendorCode ):array
  {
    ...
    $priceType = $this->options['price_set_filter'];

    $strSql = "SELECT * FROM ci_price_set WHERE ARTICLE = '{$vendorCode}' AND PRICE_TYPE = '{$priceType}'";
  ...
  private function getItemsDictionay():void
  {
    $strSql = "SELECT bitrix_id, nmid FROM wdhs_wb_props WHERE cabinet = '{$this->cabinet}'";
    $rows = $this->db->Query( $strSql );
    $result = [];
  ...
  private function getConvertedPrice( int $currencyCode, int $price ):float
  {
    $arCurrency = [
      643 => 'RUB', 840 => 'USD', 978 => 'EUR',
      933 => 'BYN', 985 => 'PLN', 156 => 'CNY',
      344 => 'HKD', 398 => 'KZT', 51 => 'AMD',
      417 => 'KGS', 376 => 'ILS', 949 => 'TRY',
      860 => 'UZS', 944 => 'AZN', 933 => 'SM'
    ];

    $c = $arCurrency[$currencyCode];
    if ( $c == 'RUB' ){
      $rate = 1;
    }else{
      $strSql = "SELECT rate FROM ci_currency WHERE id = '{$c}'";
      $rate = $this->db->Query( $strSql, false, $err_mess.__LINE__)->Fetch()['rate'] ?? 1;
    }
