...
$workers = new WorkersChecker("panel_engine_wb_analytics_topAnalytics_php");
...
class TopAnalyticsWB
{
  ...
  {
    $chunks = array_chunk( array_keys($this->items), 10, true );
  ...
  {
    $date = date('Y-m-d');
    $strSql = "DELETE FROM wb_top_analytics WHERE date ='{$date}'";
    $this->dbPanel->query( $strSql );
  }

  public function writeInDatabase():void
  {
    $this->fuckYouBitrixORM( 'wb_top_analytics', array_values($this->goods) );
  }
