...
$workers = new WorkersChecker("admin_panel_engine_ms_getTurnoverWB_php");
...
/*
	...
	public function GetTurnover()	{
		...
		for ( $i = 1; $i <= 10; $i++ ){
			...
			$msItems = $ms->getTurnoverWeekWB($i);

			foreach ($msItems as $key => $value) {

				if (!empty($value['assortment']['article']) and ($value['outcome']['quantity'] != '0') and empty($this->fromMS[$value['assortment']['article']]) ) {
					$this->fromMS[$value['assortment']['article']] = intval(($value['outcome']['sum'] / 100) / $value['outcome']['quantity']);
				}
			}
		}
	...
	private function getTurnoverPerAgent():void
	{
		...
		if ( !$suppliers ){
			// $this->bot->sendMessage("Ошибка формирования фильтра для получения отчета об остатках. Себестоимости для ВБ не обновлены");
			die("Ошибка формирования фильтра для получения отчета об остатках. Себестоимости для ВБ не обновлены");
		}
		...
		file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/admin/panel/engine/ms/fromMSWB.log', print_r($this->fromMS, true));
	...
	{
		$path = '/var/www/bitrix/data/www/tempusshop.ru/bitrix/components/adm/utils.partners/configs/135989.json';
	...
	public function PrintResult()
	{
		if (!empty($this->fromMS)) {
			$this->CurDB->Query("DELETE FROM ms_turnover_wb WHERE 1=1", false, $err_mess.__LINE__);
		}
		...
		foreach ($this->fromMS as $model => $q) {
			...
			$sql = "INSERT INTO ms_turnover_wb ($fields) VALUES ($values)";
