  ...
  public function checkItems( array $items ):array
  {
    ...
    foreach ( $items as $model => $item ) {
      ...
      if ( !$checkData['status']['abs'] || !$checkData['status']['deviation'] ){
        CommunicationService::log("Update requirements were not fulfilled: abs or deviation check failed. Item will not be updated");
      ...
      // SimpleGoalCorrector::calculateCorrection( $model, $step );
      // SimpleGoalCorrector::update( $model );
      ...
      $data = [
        ...
        'date' => date('Y-m-d G:i:s', strtotime('- 3 min')),
      ...
      CommunicationService::log("Price will be changed from {$item['installed']['price']} to {$price}");
