...
class OzonImportProducts{
	...
	public function run($modelsControl = array()){

		foreach ((array)$_SERVER['argv'] as $v){
			list($k,$v) = explode("=",$v);
			if ($k && $v) $request[$k] = $v;
		}
		$this->control = $modelsControl;
		$this->getItems();
		$this->BuildArray();
		$arTusks = $this->UploadProducts();
		return $arTusks;
	}
		...
		public function getItems(){
			...
			foreach ($this->control as $modelSingle) {
				if (!in_array($modelSingle,$checkArray)) {
					$models[]  = $modelSingle;
				}
			}
			//$models = implode(",", $models);
			//$models = array("MTP-V002L-1B");
	  ...
	  public function BuildArray() {
	    foreach ($this->items as $key => $arItem) {
				...
				foreach($att as $keyDetail => $elem){
					...
					if ($elem['id'] == 4191 ){
						$att[$keyDetail]['values'][0]['value'] = $elem['values'][0]['value']['TEXT'];
					}
					if ($elem['id'] == 8229) {
