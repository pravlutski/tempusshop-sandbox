  ...
  //текущие
	public function GetUsesProducts() {
		...
		foreach ($this->salesActive as $key => $value) {
			...
			$last_id = "";
				while ($check == true) {
					file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['GetUsesProducts', date('Y-m-d H:i:s')], true), 8);
					$data_string = json_encode(array(
						"action_id"=> $saleID, "limit" => 1000,"last_id" => $last_id
					));
	...
	public function GetPotencialProducts() {
		foreach ($this->salesActive as $key => $value) {
			...
			$last_id = "";
			while ($check == true) {
					file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['GetPotencialProducts', date('Y-m-d H:i:s')], true), 8);
				$data_string = json_encode(array(
					"action_id"=> $saleID, "limit" => 1000,"last_id" => $last_id
				));
...
public function CheckItemsAll()
{
			...
			foreach ($this->salesActive as $k => $sale) {
				if (!empty($this->arTmpResU[$sale['sale_id']]['REP'])) {
					...
					foreach ($tws as $key => $values) {
						file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['UsesReplace', date('Y-m-d H:i:s')], true), 8);
	...
	public function DeactivateSales(){
			if (!empty($this->salesActive)) {
				foreach ($this->salesActive as $k => $sale) {
						...
						foreach ($tws as $key => $values) {
							file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['DeactivateSales', date('Y-m-d H:i:s')], true), 8);
							//print_r($values);
								foreach ($values as $key => $v) {
									$tmpPrd[] = $v['ozon_id'];
								}
	...
	public function ActivateSales(){
			if (!empty($this->salesActive)) {
				foreach ($this->salesActive as $k => $sale) {
						...
						// print_r('<br>---------------------------------');
						foreach ($tws as $key => $values) {
							file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['ActivateSales', date('Y-m-d H:i:s')], true), 8);
								foreach ($values as $key => $v) {
									$tmpPrd[] = ['action_price' => $v['needed'], 'product_id'=> $v['ozon_id']];
								}
								//file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/panel/engine/ozon/logs/{$this->cabinet}/sales/log_3.txt", print_r($tmpPrd, true).PHP_EOL, FILE_APPEND);
	...
	public function DeleteNotActiveSales()	{
			foreach ($this->NotActiveSales as $key => $value) {
				...
				$last_id = "";
				while ($check == true) {
					file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['DeleteNotActiveSales', date('Y-m-d H:i:s')], true), 8);
					$data_string = json_encode(array(
						"action_id"=> $saleID, "limit" => 1000,"last_id" => $last_id
					));
					$ch = curl_init($this->api_url . '/v1/actions/products');
					...
					if (!empty($delete)) {
							...
							foreach ($tws as $key => $values) {
									...
									file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['DeleteNotActiveSales2', date('Y-m-d H:i:s')], true), 8);
			...
			foreach ($this->NotActiveProduct as $key => $value) {
				...
				foreach ($tws as $key => $values) {
									file_put_contents('/var/www/bitrix_logs/ozon/timing.txt', print_r(['NotActiveProduct', date('Y-m-d H:i:s')], true), 8);
						foreach ($values as $key => $v) {
							$tmpPrd[] = $v['product_id'];
						}
						$data_string = json_encode(array('action_id'=>$saleID ,'product_ids'=>$tmpPrd));
...
(new CLEARSALES("TI"))->UpdateTmpTable();
(new CLEARSALES("IP"))->UpdateTmpTable();
