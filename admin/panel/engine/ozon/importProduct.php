...
class OzonImportProducts{
	public function __construct($cabinet,$pdocuctID,$dislink = 0){

		if ( !in_array( $cabinet, ['TI', 'IP']) ) die("WRONG CABINET");
		if (empty($pdocuctID)) die("WRONG ID");
		global $DB;
		$this->loadModules();
		$this->pID = $pdocuctID;
	    ...
	    while ($el = $result->GetNextElement()){
				...
				if (!empty($res['PROPERTY_MORE_PHOTO_VALUE'])) {
					foreach ($res['PROPERTY_MORE_PHOTO_VALUE'] as $key => $value) {
						$tmpArrMORE[] = 'https://www.tempusshop.ru'.CFile::GetPath($value);
					}
				}
				//print_r($tmpArrMORE);
				if (!empty($res['PROPERTY_INFO_TOP_VALUE'])) {
					foreach ($res['PROPERTY_INFO_TOP_VALUE'] as $key => $value) {
						$tmpArrTOP[] = 'https://www.tempusshop.ru'.CFile::GetPath($value);
					}
				}
				//print_r($tmpArrTOP);
				$morephoto = [];
				...
				if (!empty($res['PROPERTY_INFOOZON_IMAGE_VALUE'])) {
					$mainphoto = 'https://www.tempusshop.ru'.CFile::GetPath($res['PROPERTY_INFOOZON_IMAGE_VALUE']);
				} else {
					$mainphoto = 'https://www.tempusshop.ru'.CFile::GetPath($res['PROPERTY_IMAGE_MARKETPLACE_VALUE']);
				}
				$res['MAINPHOTO'] = $mainphoto;
				if (!empty($res['PROPERTY_IMAGE_MARKETPLACE_VALUE'])) {
					$colorimg = 'https://www.tempusshop.ru'.CFile::GetPath($res['PROPERTY_IMAGE_MARKETPLACE_VALUE']);
				}
				...
				// 	$res['MOREPHOTO'][] = "https://www.tempusshop.ru/upload/wb_after_img.jpg";
				// } else {
				// 	$res['MOREPHOTO'][] = "https://www.tempusshop.ru/upload/wb_after_img_cif.jpg";
				// }
				if (!empty($res['PROPERTY_CML2_ARTICLE_VALUE'])) {
					$res['article'] = $res['PROPERTY_CML2_ARTICLE_VALUE'];
					$this->article = $res['PROPERTY_CML2_ARTICLE_VALUE'];
				}
	  ...
	  public function BuildArray() {
	    foreach ($this->items as $key => $arItem) {
						 ...
						 if ($arItem['PROPERTIES'][$propId]['PROPERTY_TYPE'] == 'F') {
							 ...
							 if (!empty($arItem['PROPERTIES'][$propId]['VALUE'])) {
								 	if (is_array($arItem['PROPERTIES'][$propId]['VALUE'])) {
										foreach ($arItem['PROPERTIES'][$propId]['VALUE'] as $k => $v) {
											if ( $v != '0' ){
												$arrImg[] = ['value'=>'https://www.tempusshop.ru'.CFile::GetPath($v)];
											}else{
												$arrImg = [];
											}
										}
									 ...
									 $att[] = array(
			  						 'id' => $attId,
			  						 'values' => array(
			  							 array('value' => 'https://www.tempusshop.ru'.CFile::GetPath($arItem['PROPERTIES'][$propId]['VALUE']))
										 )
			  						);
									}
								}
				...
				if (isset($this->checkSalePrice[$arItem['article']])) {
					$price = intval($arItem["{$this->price_prop}_VALUE"]) * 1.9;
				} else {
		...
		public function UploadProducts(){
			...
			foreach ($this->arUpdateProducts as $key => $data) {
				...
				$ch = curl_init($this->api_url . '/v3/product/import');
					...
					$this->res = $res;
					if (!empty($res['result']['task_id'])){
						$ch = curl_init($this->api_url . '/v1/product/import/info');
							curl_setopt($ch, CURLOPT_HTTPHEADER, array(
								'Api-Key:' . $this->token,
								'Client-Id:' . $this->client_id,
								'Content-Type:application/json'
							));
							curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(array('task_id' => $res['result']['task_id'])));
							curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
							curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
							curl_setopt($ch, CURLOPT_HEADER, false);
							$ans = curl_exec($ch);
...
if (isset($_GET['pid']) && !empty($_GET['pid'])) {
	$importer = new OzonImportProducts('IP', $_GET['pid']);
  $result = $importer->run();
	// print_r($result);
	?>
	<h1>Импорт товара <?=$result->article?></h1>
	<div class="flex-rc">
		<div class="flex">
			<h2>Иссходящий массив</h2>
			<div id="textarea" class="textarea"><?echo '<pre>' . print_r($result->data, true) . '</pre>';?></div>
		</div>
		<div class="flex">
			<h2>Ответ озона</h2>
			<div id="textarea" class="textarea"><?echo '<pre>' . print_r($result->res, true) . '</pre>';?><?echo '<pre>' . print_r($result->answer, true) . '</pre>';?></div>
		</div>
	</div>
	<style>
	.textarea {
