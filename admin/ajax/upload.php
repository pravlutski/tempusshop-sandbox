...
if(isset($_FILES["file_price"]) && $_FILES["file_price"]["error"] == UPLOAD_ERR_OK){
	...
	if(move_uploaded_file($_FILES['file_price']['tmp_name'], $upload_dir.$new_filename )){
		...
		//file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnie/list.txt", print_r($list , true).PHP_EOL, FILE_APPEND);
		$items = array();
		$fullname = 'Накладная от '.$dateReport;
		$strSql = "SELECT * FROM ci_nakladnie WHERE date = '{$dateReport}'";
		...
		if ($ax > 1) {
			$fullname = 'Накладная от '.$dateReport.' №'.$ax;
		}
		...
		$id_naklad_bd = $DB->Insert("ci_nakladnie", $in, $err_mess.__LINE__);
		...
		for ($i=1; $i < count($list); $i++) {
			...
			if (!empty($model)) {
				...
				$res = $DB->Insert("ci_nakladnie_pos", $in, $err_mess.__LINE__);
			}
		}
