...
<?if(!$arResult["ERROR"]):
	...
	if (isset($_POST['wh']) && $_POST['wh'] != 'ns'){
		$strSql = "SELECT * FROM ci_nakladnie WHERE STR_TO_DATE(date, '%d.%m.%Y') <= STR_TO_DATE('".$arResult['DATE_TO']."', '%d.%m.%Y') AND wh = '".$_POST['wh']."' ORDER BY date ASC";
 	} else {
	                ...
	                <span class="arrow-4-right"></span>
							...
							//create excel'гтд' => $gtd
								...
								*/
								...
								$filePath = '/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnie/templates/1.xlsx';
								...
								$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
								try {
								    $objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$name.'_post.xlsx');
								} catch (Exception $e) {
								...
								###реализация от ВР
								$filePath = '/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnie/templates/2.xlsx';
								$objPHPExcel = PHPExcel_IOFactory::load($filePath);
								$sheet = $objPHPExcel->getActiveSheet();
								...
								$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
								try {
								    $objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$name.'_r-wr.xlsx');
								} catch (Exception $e) {
								    echo 'Ошибка сохранения файла: ', $e->getMessage();
								}

								###реализация от ХГ
								$filePath = '/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnie/templates/3.xlsx';
								$objPHPExcel = PHPExcel_IOFactory::load($filePath);
								$sheet = $objPHPExcel->getActiveSheet();
								...
								$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
								try {
								    $objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$name.'_r-hg.xlsx');
								} catch (Exception $e) {
								    echo 'Ошибка сохранения файла: ', $e->getMessage();
								}
							?>
							<div>
								...
								<a href="/upload/nakladnie_cache/<?=$name?>_post.xlsx" style="margin-left:10px;" target="_blank">Поступление</a>
								<a href="/upload/nakladnie_cache/<?=$name?>_r-wr.xlsx" style="margin-left:10px;" target="_blank">Реализация ВР</a>
								<a href="/upload/nakladnie_cache/<?=$name?>_r-hg.xlsx" style="margin-left:10px;" target="_blank">Реализация ХГ</a>
...
foreach ($arNKExcel as $key => $value) {
	...
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
	$objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$key.'_ost.xlsx');
	...
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
	$objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$key.'_sell.xlsx');
	...
	<div class="row" style="background-color: rgba(134, 245, 0, 0.3); border-radius: 20px;margin-top: 15px;width:600px;padding: 10px;">
		...
		<div class="col-sm-3" style="display: flex;  justify-content: flex-end;">
			<a href="/upload/nakladnie_cache/<?=$key?>_ost.xlsx" target="_blank">Остатки</a>
		</div>
		<div class="col-sm-3" style="display: flex;  justify-content: flex-end;">
			<a href="/upload/nakladnie_cache/<?=$key?>_sell.xlsx" target="_blank">Проданное</a>
		</div>
	</div>
<?
}
    ...
    $('[data-toggle="tooltip"]').tooltip();
		$(".close").on("click", function() {
			 if (confirm("Вы действительно хотите удалить?")) {
					 ...
					 $.ajax({
							 url: "/admin/ajax/nakladnie/delete_item.php",
							 method: "POST",
								 ...
								 if (confirm("Успешно удалено.")) {
										  var dataString = $('#date_to_report').val();
											...
											$.ajax({
												...
												url: "/admin/ajax/nakladnie/get_list.php",
												cache: false,
												crossDomain: true,
												xhrFields: {
