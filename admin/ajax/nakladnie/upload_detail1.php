...
if(isset($_FILES["file_detail"]) && $_FILES["file_detail"]["error"] == UPLOAD_ERR_OK){
	...
	if(move_uploaded_file($_FILES['file_detail']['tmp_name'], $upload_dir.$new_filename )){
		...
		$id_naklad_bd = $DB->Insert("ci_nk_reports", $in, $err_mess.__LINE__);

		if ($type_report == 'wb') {
			foreach ($list as $key => $value) {


				if (($value[9] == 'Продажа' or $value[9] == 'продажа' or $value[9] == 'Возврат' or $value[9] == 'возврат') and ($value[10] == 'Продажа' or $value[10] == 'Корректная продажа' or $value[10] == 'Сторно возвратов' or $value[10] == 'Авансовая оплата за товар без движения'
				or $value[10] == 'Возврат' or $value[10] == 'Сторно продаж' or $value[10] == 'Корректный возврат')) {

					$wbArt = $value[5];
					$model = $itemsBx[$wbArt];
					$q = intval($value[13]);
					if($value[9] == 'Продажа' or $value[9] == 'Возврат') {
						$type = $value[9];
					} else if ($value[9] == 'продажа') {
						$type = 'Продажа';
					} else if ($value[9] == 'возврат') {
						$type = 'Возврат';
					}
					$price = $value[19];
					...
					// file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnie/test.txt", print_r($in, true) . "\r\n", FILE_APPEND);
					// file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnie/test.txt", print_r('###NEXT###', true) . "\r\n", FILE_APPEND);
					$res = $DB->Insert("ci_nk_reports_position", $in, $err_mess.__LINE__);
				}
