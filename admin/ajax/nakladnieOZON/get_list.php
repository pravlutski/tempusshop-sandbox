...
<?if(!$arResult["ERROR"]):
	...
	while ($row = $results->Fetch()){
		...
		// ];
		$date = $row['date'].' 00:00:00';
		if (isset($arDataResult[$date])) {
			$i= $i + 1;
			$date = $row['date'].' 00:00:'.$i.'';
		}
		...
		$arNINfo[$row['id']] = $row['name'];
		$arNDate[$row['id']] = $row['date'];
	}
  ...
  display: flex;">
	...
	<div style="margin-top: 40px;
	  margin-bottom: 20px;
	  margin-left: 10px;
	  font-size: 18px;">Сумма продаж по всем отчетам: <span style="font-weight:600;" id="top-ready"></span></div><?
	foreach($arDataResult as $key => $ar):?>
		<?if (isset($ar['REPORT'])){?>
			...
			foreach ($ar['REPORT'] as $key => $report) {
				...
				// 	}
				// }
				//file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnieOZON/test.txt", print_r($curAr, true).PHP_EOL, FILE_APPEND);
				if (count($arSklad)>0) {
			...
			$sheet->setCellValue("B1", "Тип операции");
			...
			$row = 2;
			foreach ($fullcurArr as $type => $modelArr) {
				$column = 0;
				foreach($modelArr as $model => $v){
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column, $row, $v);
							$column++;
				}
				$row++;
			}
	        ...
	        <div class="row header_table">
	            <div class="title_header_table">
								<a style="font-size:10px;" href="/upload/nakladnie_cache/<?=$name?>.xlsx" target="_blank">(Полный отчет)</a>
	              <span>Отчет коммисионера от <?=$report['date']?> <?if ($report['type'] == 'wb') {echo ' (WB) ';} else if ($report['type'] == 'oz') {echo ' (OZON) ';}?></span>
	            </div>
	            <div class="arrow-4">
	                <span class="arrow-4-left"></span>
	                <span class="arrow-4-right"></span>
	            </div>
							<?//print_r($curArPrint['Возврат']);

							//create excel'гтд' => $gtd
								$objPHPExcel = new PHPExcel();
								$objPHPExcel->setActiveSheetIndex(0);
								$sheet = $objPHPExcel->getActiveSheet();
								$title = 'Отчет от '.$report['date'].'';
								...
								$sheet->setCellValue("A2", "Модель");
								$sheet->getStyle("A2")->getFont()->setBold(true);
								$sheet->setCellValue("B2", "Кол-во");
								$sheet->getStyle("B2")->getFont()->setBold(true);
								$sheet->setCellValue("C2", "Цена/шт");
								$sheet->getStyle("C2")->getFont()->setBold(true);
								$sheet->setCellValue("D2", "Сумма");
								$sheet->getStyle("D2")->getFont()->setBold(true);
								$sheet->setCellValue("E2", "Страна");
								$sheet->getStyle("D2")->getFont()->setBold(true);
								$sheet->setCellValue("F2", "ГТД");
								$sheet->getStyle("F2")->getFont()->setBold(true);

								// $sheet->getColumnDimension("H")->setWidth(20);
								// $sheet->getColumnDimension("I")->setWidth(10);
								// $sheet->getColumnDimension("J")->setWidth(10);
								// $sheet->setCellValue("H1", "Возвраты");
								// $sheet->getStyle("H1")->getFont()->setSize(18)->setBold(true);
								// $sheet->setCellValue("H2", "Модель");
								// $sheet->getStyle("H2")->getFont()->setBold(true);
								// $sheet->setCellValue("I2", "ГТД");
								...
								// $sheet->setCellValue("L1", "Вернуть Хроносу");
								// $sheet->getStyle("L1")->getFont()->setSize(18)->setBold(true);
								// $sheet->setCellValue("L2", "Модель");
								// $sheet->getStyle("L2")->getFont()->setBold(true);
								// $sheet->setCellValue("M2", "Кол-во");
								// $sheet->getStyle("M2")->getFont()->setBold(true);

								foreach ($curArPrint as $type => $modelArr) {
									if ($type == 'Продажа') {
										$row = 3;
											if (count($modelArr) > 0) {
												foreach($modelArr as $model => $v2){
														foreach($v2 as $gtd => $v) {
												    $column = 0;
													    foreach ($v as $dataValue) {
													        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column, $row, $dataValue);
													        $column++;
													    }
															$row++;
														}
												...
												$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0,3, 'Склад пуст на момент отчета');
									...
									// 			foreach($modelArr as $model => $v){
									// 					$column = 12;
									// 					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column-1, $row, $model);
									// 					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column, $row, $v['quantity']);
									// 					$row++;
									// 			}
									// 		} else {
									// 			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11,3,' Возврат хроносу в отчете отсутствуют');
									// 			$sheet->getStyle("L3")->getFont()->getColor()->setRGB('ff0000');
									// 		}
									//
									// }

									foreach ($modelArr as $model => $values) {
										foreach($values as $gtd => $v):
											$summAll0 = $summAll0 + floatval($v['sum']);
										endforeach;
									}
									foreach ($modelArr as $model => $values) {
										foreach($values as $gtd => $v):
											$summAll = $summAll + floatval($v['sum']);
										endforeach;
									}
								}
								$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
								...
								*/
								...
								$filePath = '/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnieOZON/templates/1.xlsx';
									...
									if ($type == 'Продажа') {
										$row = 2;
											if (count($modelArr) > 0) {
												foreach($modelArr as $model => $v2){
														foreach($v2 as $gtd => $v) {
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $v['name']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $v['q']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $v['price']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $v['sum']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row, $v['country']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $gtd);
															$row++;
														}
												}
											}
								...
								$sheet->setAutoFilter('');
								// Сохранение изменений в существующий файл или в новый файл
								$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
								try {
								    $objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$name.'_ozon_post.xlsx');
								} catch (Exception $e) {
								    echo 'Ошибка сохранения файла: ', $e->getMessage();
								}


								###реализация от ВР
								$filePath = '/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnieOZON/templates/2.xlsx';
								$objPHPExcel = PHPExcel_IOFactory::load($filePath);
								$sheet = $objPHPExcel->getActiveSheet();
											...
											if (count($modelArr) > 0) {
												foreach($modelArr as $model => $v2){
														foreach($v2 as $gtd => $v) {
															...
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $v['q']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $v['price']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $v['sum']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row, $v['country']);
															$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $gtd);
															$row++;
														}
								...
								$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
								try {
								    $objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$name.'_ozon_r-wr.xlsx');
								} catch (Exception $e) {
								    echo 'Ошибка сохранения файла: ', $e->getMessage();
								}

								###реализация от ХГ
								$filePath = '/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnieOZON/templates/3.xlsx';
								$objPHPExcel = PHPExcel_IOFactory::load($filePath);
								$sheet = $objPHPExcel->getActiveSheet();
								...
								$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
								try {
								    $objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$name.'_ozon_r-hg.xlsx');
								} catch (Exception $e) {
								    echo 'Ошибка сохранения файла: ', $e->getMessage();
								}
							?>
							<div>
								<a href="/upload/nakladnie_cache/<?=$name?>.xlsx" target="_blank">Скачать excel</a>
								<span style="font-size: 14px; font-weight: normal;">(<b>Сумма прожаж:</b> <?=number_format(round($summAll,2), 1, '.', ' ');?>)</span>
								<a href="/upload/nakladnie_cache/<?=$name?>_ozon_post.xlsx" style="margin-left:10px;" target="_blank">Поступление</a>
								<a href="/upload/nakladnie_cache/<?=$name?>_ozon_r-wr.xlsx" style="margin-left:10px;" target="_blank">Реализация ВР</a>
								<a href="/upload/nakladnie_cache/<?=$name?>_ozon_r-hg.xlsx" style="margin-left:10px;" target="_blank">Реализация ХГ</a>
								<?unset($summAll);?>
							</div>
							<div class="close" data-type="report" data-del="<?=$report['id']?>"></div>
	        </div>
				<div class="row table">
					<?foreach ($curArPrint as $type => $modelArr) {?>
						<?if ($type == 'Продажа') {?>
							...
							<div class="col-sm-12">
								<h3>Продажа</h3>
								<?if (count($modelArr) <= 0) {?>
									<span style='color:red'>Склад пуст на момент отчета</span>
								<?}else {?>
								<table class="">
								<thead>
									<tr>
		...
		<?if (isset($ar['SKLAD_UPDATE'])){?>
			...
			<?
				...
				$objPHPExcel->setActiveSheetIndex(0);
				$sheet = $objPHPExcel->getActiveSheet();
				$dateN = explode(' ',$ar['SKLAD_UPDATE']['date']);
				$title = 'Пополнение склада из наклодной'.$date[0].'';
				$sheet->setTitle($title);
				$sheet->getColumnDimension("A")->setWidth(20);
				$sheet->getColumnDimension("B")->setWidth(20);
				$sheet->getColumnDimension("C")->setWidth(10);
				$sheet->setCellValue("A1", "Пополнение");
				$sheet->getStyle("A1")->getFont()->setSize(18)->setBold(true);
				$sheet->setCellValue("A2", "Модель");
				$sheet->getStyle("A2")->getFont()->setBold(true);
				$sheet->setCellValue("B2", "ГТД");
				$sheet->getStyle("B2")->getFont()->setBold(true);
				$sheet->setCellValue("C2", "Кол-во");
				$sheet->getStyle("C2")->getFont()->setBold(true);

				$row = 3;
				foreach ($arDifPrint as $model => $gtdS) {

						if (count($gtdS) > 0) {
							$column = 0;
							foreach($gtdS as $gtd => $skld){
								foreach($skld as $mnm){
									if ($mnm['q'] != 0) {
										$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column, $row, $mnm['name']);
										$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column+1, $row, $gtd);
										$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column+2, $row, $mnm['q']);
										$row++;
									}
								}
							}
        ...
        <div class="row header_table">
            <div class="title_header_table">
              <span>Пополнение склада из наклодной "<?=$ar['SKLAD_UPDATE']['name']?>"</span>
            </div>
            <div class="arrow-4">
                <span class="arrow-4-left"></span>
                <span class="arrow-4-right"></span>
						...
						<div class="close" data-type="nakl" data-del="<?=$ar['SKLAD_UPDATE']['id']?>"></div>
        </div>
				<?//print_r($ar['SKLAD_UPDATE']);?>
				<div class="table">
				<h4>Текущее состояние склада</h3>
				<table class="">
					<thead>
						<tr>
							<th style="width: 15%">Модель</span></th>
							<th style="width: 15%">ГТД</span></th>
							<th style="width: 15%">Страна</span></th>
							<th style="width: 15%">Кол-во</th>
							<th style="width: 15%">Изм.</th>
						</tr>
					</thead>
					<tbody>
						<?foreach($arSklad as $model => $nkid):?>
							<?foreach($nkid as $tmplnkid => $q):?>
							<?foreach($q as $gtd => $rmv):?>
							<?foreach($rmv as $vls):?>
							<?if (isset($arDifTmp[$model][$gtd]) && count($arDifTmp[$model][$gtd]) > 0){?>
								<?foreach($arDifTmp[$model][$gtd] as $ddn):?>
								<tr class="">
									<td><?=$vls['name']?></td>
									<td><?if ($gtd == '') { echo "отсутствует"; } else { print_r($gtd); }?></td>
									<td><?if ($vls['country'] == '') { echo "отсутствует"; } else { print_r($vls['country']); }?></td>
									<td><?=$vls['quantity']?></td>
									<td>+ <?=$ddn?></td>
								</tr>
								<?endforeach?>
...
?>
	<?endforeach?>
<?endif;?>
<span class="itogo"><?=number_format(round($summAll0,2), 1, '.', ' ');?></span>
<?//print_r($arSklad);?>
<?//file_put_contents("/var/www/bitrix/data/www/tempusshop.ru/admin/ajax/nakladnieOZON/sl.txt", print_r($tmpSellCycle, true).PHP_EOL, FILE_APPEND);?>
</div>
<?if (!empty($arNKExcel)) {?>
<div class="row">
	<div style="border-bottom:1px solid #111; height:1px;"></div>
</div>
<?}?>
<?
//print_r($arNKExcel[260]);
foreach ($arNKExcel as $key => $value) {

	$nameNaklad = $arNINfo[$key];

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->setActiveSheetIndex(0);
	$sheet = $objPHPExcel->getActiveSheet();
	$title = 'Остатки из накладной '. $arNDate .'';
	$sheet->setTitle($title);
	$sheet->getColumnDimension("A")->setWidth(20);
	$sheet->getColumnDimension("B")->setWidth(20);
	$sheet->getColumnDimension("C")->setWidth(10);
	...
	$sheet->getStyle("D1")->getFont()->setBold(true);
	$sheet->setCellValue("E1", "Номер EXCEL");
	$sheet->getStyle("E1")->getFont()->setBold(true);

	$row = 2;

	foreach ($value['HAVE'] as $model => $gtdS) {
			if (count($gtdS) > 0) {
				$column = 0;
				foreach($gtdS as $gtd => $skld){
						if ($skld['quantity'] != 0) {
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column, $row, $skld['name']);
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column+1, $row, $skld['quantity']);
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column+2, $row, $gtd);
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column+3, $row, $skld['country']);
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column+4, $row, $skld['row_excel']);
							$row++;
						}
				}
			}
	...
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
	$objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$key.'_ost_ozon.xlsx');
	...
	$sheet = $objPHPExcel->getActiveSheet();
	$dateN = explode(' ',$ar['SKLAD_UPDATE']['date']);
	$title = 'Продано из накладной '. $arNDate .'';
	$sheet->setTitle($title);
	$sheet->getColumnDimension("A")->setWidth(20);
	$sheet->getColumnDimension("B")->setWidth(20);
	$sheet->getColumnDimension("C")->setWidth(10);
	...
	foreach ($value['SELL'] as $model => $gtdS) {
	...
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
	$objWriter->save('/var/www/bitrix/data/www/tempusshop.ru/upload/nakladnie_cache/'.$key.'_sell_ozon.xlsx');

	?>

	<div class="row" style="background-color: rgba(134, 245, 0, 0.3); border-radius: 20px;margin-top: 15px;width:600px;padding: 10px;">
		...
		<div class="col-sm-3" style="display: flex;  justify-content: flex-end;">
			<a href="/upload/nakladnie_cache/<?=$key?>_ost_ozon.xlsx" target="_blank">Остатки</a>
		</div>
		<div class="col-sm-3" style="display: flex;  justify-content: flex-end;">
			<a href="/upload/nakladnie_cache/<?=$key?>_sell_ozon.xlsx" target="_blank">Проданное</a>
		</div>
	</div>
<?
}
...
$(document).ready(function(){
	var spanValue = $('span.itogo').text();
	$('#top-ready').text(spanValue);
    $('[data-toggle="tooltip"]').tooltip();
		$(".close").on("click", function() {
			 if (confirm("Вы действительно хотите удалить?")) {
				 	 var type = $(this).attr("data-type");
					 var dataDelValue = $(this).attr("data-del");
					 $.ajax({
							 url: "/admin/ajax/nakladnieOZON/delete_item.php",
							 method: "POST",
							 data: { delValue: dataDelValue, type:type},
							 success: function(response) {
								 if (confirm("Успешно удалено.")) {
										  var dataString = $('#date_to_report').val();
											$('#popup-stocks').addClass('active-pp');
											$.ajax({
												type: "POST",
												data: {date_to:dataString},
												url: "/admin/ajax/nakladnieOZON/get_list.php",
												cache: false,
												crossDomain: true,
												xhrFields: {
												...
												success: function(data) {
													$("#result-purchase").html(data);
													$('#popup-stocks').removeClass('active-pp');
												},
												error:function(data) {
