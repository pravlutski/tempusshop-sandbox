...
//	$start = debug_microtime_float();
	...
	if (is_array($orderIds) && count($orderIds) > 0) {
		...
		while ($row = $results->Fetch()){
			...
			$arAllOrder[$row["ORDER_ID"]]['TRADING_PLATFORM_ID'] = $row["TRADING_PLATFORM_ID"];
			$arAllOrder[$row["ORDER_ID"]]['TRADING_PLATFORM_NAME'] = $tpList[$row["TRADING_PLATFORM_ID"]];
		}
	}
	//prent($arAllOrder);
	...
	foreach($arResult["ITEMS"] as $key => &$arItem){
		...
		if(isset($arAllOrder[$arItem["order_id"]])){
			...
			$arItem["tp_name"] = $arOrder["TRADING_PLATFORM_NAME"] ?? '';
...
//		$arSort[$arItem["supp_id"]] = $arItem["supp_id"];
	...
	$txt_all = "";
	if($arResult["ACCESS"] === true) $cntCol = 7; else $cntCol = 5;
	?>
		...
		<div class="col-sm-12">
			...
			<table class="table purchase-item" id="purchase-item-<?=$key?>" data-id="<?=$key?>">
				<thead>
					...
					<tr>
						<th colspan="3">Цена (<? echo number_format($arResult["PRICE_GROUP_SUM"][$key], 0, '', ' ');?>)</th>
						<th colspan="4">
              ...
              <?/*<button type="button" class="btn btn-primary " id="ms_create_docs_144" data-id="<?=$key?>" style="float: right;padding: 0px 3px 0 3px;font-size: 12px;">Создать документы</button>*/?>
              <?elseif ($key == 124): // Поставщик Олег ?>
              <button type="button" class="btn btn-primary " id="ms_create_docs_124" data-id="<?=$key?>" style="float: right;margin-right: 5px;padding: 0px 3px 0 3px;font-size: 12px;">Создать документы</button>
...
site_id
          ...
          <?//print_r($arPrice);?>
					...
					<?$arTxtAll[$arResult["SUPPLIER_NAME"][$key]][$arPrice["model"]] += 1;?>
						...
						<tr class="<?if(($arPrice["order_id"] > 0 && in_array($arPrice["order_status_id"], array("WT", "LP", "DP", "NZ", "RD", "NA", "DB", "AB", "NO", "OT", "OS", "CA", "CS"))) || $arPrice["order_canceled"] == "Y" || $arPrice["item_active"] == "N"):?>danger<?elseif($arPrice["top_id"] == 0 && ($arPrice["order_id"] > 0 && !in_array($arPrice["order_status_id"], array("CO", "SE", "TA", "PO")))):?>warning11<?elseif($arPrice["top_id"] > 0):?>success<?endif?>" data-orderbasketid="<?=$arPrice["order_basket_id"]?>" data-product_id="<?=$arPrice["product_id"]?>" data-article="<?=$arPrice["model"]?>" data-id="<?=$arPrice["id"]?>">
							...
							<td><?if($arPrice["order_id"] > 0):?><a href="https://tempusshop.ru/bitrix/admin/sale_order_view.php?amp%3Bfilter=Y&%3Bset_filter=Y&lang=ru&ID=<?=$arPrice['order_id']?>" target="_blank" style="position: relative;"><span><?=$arPrice["order_number_id"]?></span></a><?else:?><?=$arPrice["order_number_id"]?><?endif?></td>
  ...
  <div class="col-sm-12 row">
  <form action="/admin/ajax/purchase/get_purchase_by_order_csv.php" style="position: relative; margin-bottom:30px; display:flex; justify-content:spaceBetween;margin-top: 0px;justify-content: space-between;
  gap: 30px;
  padding: 20px;" method="GET">
