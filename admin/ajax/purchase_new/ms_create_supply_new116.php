...
class PurchaseMS{
    ...
    $this->supplier = new CPanelSupplier;
		...
		$this->arMS = array(
			"chronos" => array(
				"contragent" => array(
					...
					"tvoya-idea" => "0f31eb40-7b0e-11ef-0a80-0cd400179974",
				),
				"organization" => array(
					//"chronos" => "83be7e8b-0f74-11ee-0a80-143a0014a100",
					"chronos" => "2852bdf4-12d8-11ee-0a80-0bfe001f917d",
				),
			),
			"int" => array(
				"contragent" => array(
						//"chronos" => "01c31e04-4d69-11ed-0a80-0238002481b5",
						"chronos" => "1d2f4dd3-b6bf-11ef-0a80-0da400045a01",
					),
					"organization" => array(
						...
						"tvoya-idea" => "655c12e4-ae44-11ee-0a80-08ce006d3fdc",
					),
		...
		if(in_array($this->supplierID, array(116))){
	...
	public function getProduct(){
		...
		while ($row = $results->Fetch()){
            ...
            if ($row["SITE_ID"] == 'msk') {
				...
				foreach ($res as $value) {
                    if(
                        $value['storeId'] == 'e1146ee2-f19c-11ee-0a80-05c8000bd91c' ||
		...
		/*foreach ($this->arProductID as $pr_id => $site_id) {
			...
			foreach ($res as $value) {
				if($value['storeId'] == 'e1146ee2-f19c-11ee-0a80-05c8000bd91c' || $value['storeId'] == '796d5aa2-bab0-11ee-0a80-03440010c9e0' || $value['storeId'] == 'e7c0d649-55ef-11ee-0a80-1186002ba09f')
	...
	public function sendMS(){
		...
		foreach ($this->purchaseList as $key => $value) {
			if (isset($this->stockArray[$value['product_id']])) {
				...
				$sklad = 'e1146ee2-f19c-11ee-0a80-05c8000bd91c';
			}
			if ($value['site_id'] == 's1') {
				if ($value['source'] == 'wb'){
		...
		foreach ($tmpProductsArr as $sklad => $valueArray) {
			...
			if (isset($valueArray['s2']['other']) && count($valueArray['s2']['other']) > 0){
				...
				$templateSupplyS1 = $this->ms->send("/entity/supply/new", "PUT", [], ["Content-Type" => "application/json"]);
				print_r('ot_s2');
				$arPosition = $this->preparePositions("s1",$valueArray['s2']['other']);
				...
				$templateSupplyS1["organization"] = array(
					"meta" => array(
						"href" => "https://api.moysklad.ru/api/remap/1.2/entity/organization/" . $this->arMS["int"]['organization']['tvoya-idea'],
						"metadataHref" => "https://api.moysklad.ru/api/remap/1.2/entity/organization/metadata",
						"type" => "organization",
						"mediaType" => "application/json"
					)
				...
				if ($doc) {
					...
					$templateDemand["agent"] = array(
						"meta" => array(
							"href" => "https://api.moysklad.ru/api/remap/1.2/entity/counterparty/" . $this->arMS["chronos"]['contragent']['tvoya-idea'],
							"type" => "counterparty",
							"mediaType" => "application/json"
						)
					);
