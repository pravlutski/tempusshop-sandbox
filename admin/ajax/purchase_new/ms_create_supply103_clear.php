...
class PurchaseMS{
	...
	public function sendMS(){
		...
		*/
		...
		if($this->supplierID == 47){
			...
			// создаем приёмки
			$this->createDocument("s1", "supply", $supp["ms_id"]);

			// создаем возвраты
			$currency = $this->currency->getDetail("BYN");//курс валюты
			$this->rate = $currency["rate"];
			$msAgentID = "236a0c9f-35dd-11ea-0a80-0217000fedca";
	...
	public function createDocument($ms_cabinet = "s1", $type = "supply", $msAgentID = ""){
		...
		$template['description'] = 'для проверки комплектации склада Темпуса';
	...
	public function prepareTemplate(&$template, $ms_cabinet = "s1", $ms_id = ""){
