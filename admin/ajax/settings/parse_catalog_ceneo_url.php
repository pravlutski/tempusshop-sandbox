...
exec("pgrep -f /userscripts/parse_ceneo_url.php",$output,$code); 
if(is_array($output) && count($output) > 0){
	foreach($output as $pid)
		exec("kill -9 {$pid}");
}
CProSet::setOption("PARSE_CENEO_URI", "Y");
system("/usr/bin/php81 -f /userscripts/parse_ceneo_url.php >/dev/null 2>&1 &");
