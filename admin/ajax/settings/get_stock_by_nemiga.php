...
//
if(!CModule::IncludeModule('panel.manager') || !CModule::IncludeModule("iblock") || !CModule::IncludeModule("main") || 
$_SERVER['HTTP_X_REQUESTED_WITH'] != 'XMLHttpRequest') return;
/*
$obj = new CExchange("s2");
$result = $obj->updateFromGoogleDocs(true);
*/
//получаем минские из системы МойСклад
$obj = new CExchange("s2_nemiga");
...
//	system("/usr/bin/php81 -f /userscripts/update_catalog_all.php >/dev/null 2>&1 &");
	$txt = "Минск - " . $result["info"] . ". Обновлено - " . $result["cnt"];
