...
class DocsCreate124
{
  ...
  private $organization; // Организации
  private $counterparty; // Контрагенты
  private $applicable; // Проведен ли документ
  ...
  public function __construct()
  {
    ...
    $this->description = [
      's2_rest' => 'Август розница',
      's2_21vek' => 'Август 21 век',
      's1_wr' => 'Август темпус',
    ];
  ...
  private function processDocuments():void
  {
    ...
    foreach ( $items as $key => $arProducts ){
      if (!empty($arProducts)) {
        if ( in_array( $key, ['s2_rest', 's2_21vek'] ) ){
          $config = [
            'organization' => $this->organization['msk']['chronos'],
            'store' => 'e88d8276-f19c-11ee-0a80-044a000d1702',
            'agent' => $this->counterparty['msk']['watch_trade'],
            'description' => $this->description[$key],
            'applicable' => $this->applicable['demand'][$key],
          ];
          $this->createDocument( 'msk', 'demand', $arProducts, $config );
          continue;
        }
        ...
        $config = [
          'organization' => $this->organization['msk']['chronos'],
          'store' => 'e88d8276-f19c-11ee-0a80-044a000d1702',
          'agent' => $this->counterparty['msk']['watches_retail'],
          'description' => $this->description[$key],
          'applicable' => $this->applicable['demand'][$key],
        ];
        $this->createDocument( 'msk', 'demand', $arProducts, $config );
        ...
        $config = [
          ...
          'agent' => $this->counterparty['s1']['chronos'],
          'description' => $this->description[$key],
          'applicable' => $this->applicable['supply'][$key],
        ];
        $this->createDocument( 's1', 'supply', $arProducts, $config );
      }
    }
  }
  ...
  private function createDocument( string $cabinet, string $docType, array $products, array $profile ):void // Формируем шаблон и отправляем в МС
  {
