...
class DocsCreate141
{
  ...
  public function __construct()
  {
    ...
    $this->description = [
      's2_rest' => 'Импорт розница',
      's2_21vek' => 'Импорт 21 век',
      's1_wr' => 'Импорт темпус',
    ];
      ...
      if (!empty($arProducts)) {
        if ( in_array( $key, ['s2_rest', 's2_21vek'] ) ){
          $config = [
            ...
            'store' => 'e1146ee2-f19c-11ee-0a80-05c8000bd91c',
            'agent' => $this->counterparty['msk']['watch_trade'],
            'description' => $this->description[$key],
            'applicable' => true,
          ];
           $op1 = $this->createDocument( 'msk', 'demand', $arProducts, $config );
          // print_r('111');
          // print_r($op1);
          continue;
        }
        ...
        $config = [
          ...
          // print_r('333');
        $op2 = $this->createDocument( 's1', 'supply', $arProducts, $config );
        // print_r($op2);
      }
    }
  ...
  private function createDocument( string $cabinet, string $docType, array $products, array $profile ) // Формируем шаблон и отправляем в МС
  {
