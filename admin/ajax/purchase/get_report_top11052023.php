...
// проверяем загружены ли склады
$arStockCode = ["UPDATE_STOCK_RU","UPDATE_STOCK_BY","UPDATE_STOCK_PL","UPDATE_STOCK_RU_OPT"];
...
if(CModule::IncludeModule("panel.manager") && CModule::IncludeModule("iblock") && CModule::IncludeModule("catalog")){
	...
	while ($row = $results->Fetch()){
		...
		if(!$arPurchase[$row["id"]]){
			$arResult["ITEMS"][] = array(
				...
				"QUANTITY" => ceil(($row["sell_quantity"] / (365 / 6))),
			);
	...
	if(is_array($arArt) && count($arArt) > 0){
		$strSql = "SELECT * FROM ci_price WHERE model IN ('".implode("','",$arArt)."') {$add_filter}";// AND supplier_id <> '{$supp_id}'";
...
//prent($arResult["ITEMS"]);
	...
	foreach($arResult["ITEMS"] as $key => &$arItem){
		...
		foreach($arPrice[$arItem["ARTICLE"]] as $k => $row){
			...
			if(in_array($row["supplier_id"], array(44,47,71,82,103))){
		...
		if(is_array($arr) && count($arr) > 0){
			foreach($arr as $k => $v){
				if($arMinPrice[$v["model"]] && $arResult["PRICE_DEVIATION"]){
					...
					if($diff <= $arResult["PRICE_DEVIATION"] || in_array($v["supplier_id"], array(44,47,71,103))){
						$arItem["DEVIATION"][] = $v;
						$last = $k;
					}
				}
