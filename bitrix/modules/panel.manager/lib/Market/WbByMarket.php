...
class WbByMarket extends AbstractMarket
{
	...
	protected function getSebesFbo (): array
	{
		...
		if (!empty($conditions)) {
			$sql = "SELECT * FROM {$this->config['tbl_sebes_fbo']} WHERE " . implode(' AND ', $conditions);
		} else {
			$sql = "SELECT * FROM {$this->config['tbl_sebes_fbo']}";
		}
	...
	protected function prepareSebesFbo ($prices = []): array 
	{
		...
		foreach ($prices as $price) {
			$arPrice[$price['article']] = [
				...
				'supplier_id' => 'sebes WB',
				'supplier_name' => 'sebes WB',
				'price' => $price['cost'],
				'count' => 100,
			];
		}
    ...
    public function applySpecificCoefficients(float $price): float
    {
        $salePercent = (float)\CProSet::getOption("CATALOG_SALE_{$this->marketCode}", 0);
        $promoPercent = (float)\CProSet::getOption("CATALOG_PROMO_{$this->marketCode}", 0);
        
        if ($salePercent > 0) {
            $price = $price * (100 / (100 - $salePercent));
        }
        
        if ($promoPercent > 0) {
            $price = $price * (100 / (100 - $promoPercent));
        }
