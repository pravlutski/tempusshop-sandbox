...
class CYandexParser{
	...
	function getPrices(){
		...
		$strSql = "SELECT product_id, price_discount_ru FROM ci_price_catalog";
			
		$results = $DB->Query($strSql, false, $err_mess.__LINE__);
		$arPrice = array();
		while ($arFields = $results->Fetch()){
			$arPrice[$arFields["product_id"]] = $arFields["price_discount_ru"];
		}
		return $arPrice;
	}
	...
	function parse(){
		...
		//if(true){
			...
			if($csv->error != "no_found"){
				...
				$arMarketParser = $arMarketParserSkip = array();
				...
				foreach($arCsv as $key => &$arItem){
					...
					//	$arMarketParser[] = $id;
					//изменили 21012022 . пишим только айди которые парсить не надо
					if($last_price <= 0 && $arItem[$col_url]) 
						$arMarketParserSkip[] = $id;
					...
					foreach($arColShop as $shop){
						if($arItem[$shop]){
							...
							$break = false;
							if($name == "Яндекс.Маркет" && $arPriceCatalog[$id] == (float)$arItem[$shop + 1]){
								$break = true;
								//prent($arItem);die;
							}
							
							//ищем минимальную цену с учетом магазинов которые надо исключить
							if(!in_array($name, $arResult["YMARKET_SHOP_HIDE"]) && $break == false){
				...
				$this->setMarketParser($arMarketParserSkip);
