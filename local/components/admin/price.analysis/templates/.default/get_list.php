...
<?if(!in_array($psFilter["website"], array("s1", "s2"))):?>
	...
	<?die;?>
<?endif?>
<?if(count_($psFilter["brand_id"]) <= 0):?>
	<tr><td colspan="7">
		<p>Выберите бренд</p>
	</tr></td>
	<?die;?>
<?endif?>
<?if(count_($psFilter["supplier_id"]) <= 0):?>
	<tr><td colspan="7">
...
*/
	...
	foreach($arPrice as $key => $arItem):?>
		...
		<tr class="<?if(!$arItem["b_price"] || $arItem["b_price"] == 0):?>no-price<?endif?> <?=$class?>" data-productid="<?=$arItem["b_id"]?>" data-bprice="<?=$arItem["b_price"]?>">
			...
			<td class="c-competitors <?if($sort == "price_platform_kz"):?>active<?endif?>"><?=$arItem["price_platform_kz"]?></td>
			<td class="c-competitors <?if($sort == "price_platform_ozkz"):?>active<?endif?>"><?=$arItem["price_platform_ozkz"]?></td>
