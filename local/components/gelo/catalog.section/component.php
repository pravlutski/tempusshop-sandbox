...
/*************************************************************************
			Work with cache
*************************************************************************/
//prent($GLOBALS["MAX_SMART_FILTER"]);
//prent($arParams);
$cacheId = md5($_REQUEST["PAGEN_1"] . serialize($GLOBALS[$arParams["FILTER_NAME"]]) . serialize($arParams));
$cachePath = "/" . SITE_ID . "/gelo/catalog.section/" . $arParams["SECTION_ID"] . "/" . $cacheId;
...
if ($obCache->InitCache($arParams["CACHE_TIME"], $cacheId, $cachePath)){
	...
	//Достаем CACHE_MANAGER из глобалки и привязываем его к нашей директории кеша
	global $CACHE_MANAGER;
	$CACHE_MANAGER->StartTagCache($cachePath);

	//Вешаем тег на текущий кеш. У нас тут будет выборка элементов инфоблока каталога
	//поэтому имеет смысл зарегистрировать тег, зависящий от ID инфоблока каталога
	//$CACHE_MANAGER->RegisterTag('iblock_id_' . $arParams["IBLOCK_ID"]);

	$CACHE_MANAGER->RegisterTag('iblock_section_id_' . $arParams["SECTION_ID"]);
...
//if($USER->isAdmin()) $arSort = array();
	...
	if($arSort["sort"]){
		switch(SITE_ID){
			case "s1":
				$arSort = array("SORT_RU" => "asc");
				break;
			case "s2":
				$arSort = array("SORT_BY" => "asc");
				break;
			case "s3":
				$arSort = array("SORT_PL" => "asc");
				break;
			default:
				$arSort = array("SORT_RU" => "asc");
				break;
	...
	if($arSort["SHOWS"]){
		switch(SITE_ID){
			case "s1":
				$arSort = array("PROPERTY_AVAILABILITY_RU" => "asc,nulls","SHOW_COUNTER" => $arSort["SHOWS"]);
				break;
			case "s2":
				$arSort = array("PROPERTY_AVAILABILITY_BY" => "asc,nulls","SHOW_COUNTER" => $arSort["SHOWS"]);
				break;
			case "s3":
				$arSort = array("PROPERTY_AVAILABILITY_PL" => "asc,nulls","SHOW_COUNTER" => $arSort["SHOWS"]);
				break;
			default:
				$arSort = array("PROPERTY_AVAILABILITY_RU" => "asc,nulls","SHOW_COUNTER" => $arSort["SHOWS"]);
				break;
	...
	//file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/local/components/gelo/catalog.section/sort.txt', print_r('---', true) . "\r\n", FILE_APPEND | LOCK_EX);
	//file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/local/components/gelo/catalog.section/sort.txt', print_r($arSort, true) . "\r\n", FILE_APPEND | LOCK_EX);
 // $arParams["ELEMENT_SORT_FIELD2"] == 'name';
 // $arParams["ELEMENT_SORT_ORDER2"] == 'asd';
	// 	 file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/local/components/gelo/catalog.section/sort.txt', print_r('---', true) . "\r\n", FILE_APPEND | LOCK_EX);
	//  	file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/local/components/gelo/catalog.section/sort.txt', print_r($arParams["ELEMENT_SORT_FIELD"], true) . "\r\n", FILE_APPEND | LOCK_EX);
	//  	file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/local/components/gelo/catalog.section/sort.txt', print_r($arParams["ELEMENT_SORT_ORDER"], true) . "\r\n", FILE_APPEND | LOCK_EX);
	//  	file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/local/components/gelo/catalog.section/sort.txt', print_r($arParams["ELEMENT_SORT_FIELD2"], true) . "\r\n", FILE_APPEND | LOCK_EX);
	//  	file_put_contents('/var/www/bitrix/data/www/tempusshop.ru/local/components/gelo/catalog.section/sort.txt', print_r($arParams["ELEMENT_SORT_ORDER2"], true) . "\r\n", FILE_APPEND | LOCK_EX);
	//prent($arSort);

	$rsElements = \CIBlockElement::GetList($arSort, array_merge($arrFilter, $arFilter), false, $arNavParams, array("ID"));
	while($arItem = $rsElements->GetNext()){
		$arIDs[] = $arItem["ID"];
	}

	if(count($arIDs) > 0){
		$arFilterID = array("=ID" => $arIDs);
	}else{
		$arFilterID = array("=ID" => 0);
	}

	$rs = \CIBlockElement::GetList($arSort, $arFilterID, false, false, $arSelect);
	//prent(array_merge($arrFilter, $arFilter));
	$rs->SetUrlTemplates($arParams["DETAIL_URL"]);
	if(
		$arParams["BY_LINK"]!=="Y"
	...
	while($arItem = $rs->GetNext())
	{
		...
		$intKey++;
		//prent($arItem['ID']);
		//Можно пометить кеш тегами всех товаров каталога
		$CACHE_MANAGER->RegisterTag('product_' . $arItem['ID']);
...
//
	...
	$uri = new \Bitrix\Main\Web\Uri($this->request->getRequestUri());
	$navComponentParameters["BASE_LINK"] = $uri->getPath();
	...
	if (!empty($arResult["ELEMENTS"]) && ($bGetProperties || ($bCatalog && $boolNeedCatalogCache)))
	{
		...
		CIBlockElement::GetPropertyValuesArray($arElementLink, $arParams["IBLOCK_ID"], $arPropFilter, array('CODE' => array('MORE_PHOTO', 'AVAILABILITY_KZ')));
//prent($arElementLink); die;
...
/*
	...
	//вызываем функцию которая удаляет бляцкий ненужный тег
	clearBadTag($cachePath);
