...
<?if($arResult["ITEMS"]):?>
	...
	<?$bShowBtn = (isset($arParams['SHOW_BTN']) && $arParams['SHOW_BTN'] == 'Y');?>
		...
		<?endif;?>
		<div class="block-items<?=($bRow ? ' flexbox flexbox--row flex-wrap' : '');?><?=($bSlide ? ' owl-carousel owl-theme owl-bg-nav short-nav hidden-dots' : '');?> swipeignore"<?if($bSlide):?>data-plugin-options='{"nav": true, "autoplay" : false, "autoplayTimeout" : "3000", "margin": -1, "smartSpeed":1000, <?=(count_($arResult["ITEMS"]) > 4 ? "\"loop\": true," : "")?> "responsiveClass": true, "responsive":{"0":{"items": 1},"600":{"items": 2},"768":{"items": 3},"992":{"items": 4}}}'<?endif;?>>
											...
											<div class="js_price_wrapper">
												<?if($arCurrentSKU && !$bOutOfProduction):?>
													...
													if(isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX'] && $arCurrentSKU['ITEM_PRICE_MODE'] == 'Q'): // USE_PRICE_COUNT?>
														...
														<?endif;?>
														<?if($arCurrentSKU['ITEM_PRICE_MODE'] == 'Q' && count_($arCurrentSKU['PRICE_MATRIX']['ROWS']) > 1):?>
									...
									<?else:?>
										...
										<?else:?>
											<?if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX']): // USE_PRICE_COUNT?>
												<?if(\CMax::GetFrontParametrValue('SHOW_POPUP_PRICE') == 'Y' || $arItem['ITEM_PRICE_MODE'] == 'Q' || (\CMax::GetFrontParametrValue('SHOW_POPUP_PRICE') != 'Y' && $arItem['ITEM_PRICE_MODE'] != 'Q' && count_($arItem['PRICE_MATRIX']['COLS']) <= 1)):?>
													<?=CMax::showPriceRangeTop($arItem, $arParams, Loc::getMessage("CATALOG_ECONOMY"));?>
												<?endif;?>
												<?if(count_($arItem['PRICE_MATRIX']['ROWS']) > 1 || count_($arItem['PRICE_MATRIX']['COLS']) > 1):?>
													<?=CMax::showPriceMatrix($arItem, $arParams, $strMeasure, $arAddToBasketData);?>
												<?endif;?>
											<?elseif(isset($arResult["PRICES"])):?>
												<?\Aspro\Functions\CAsproMaxItem::showItemPrices(array_merge((array)$arParams, (array)$addParams), $arItem["PRICES"], $strMeasure, $min_price_id, ($arParams["SHOW_DISCOUNT_PERCENT_NUMBER"] == "Y" ? "N" : "Y"));?>
											<?endif;?>
										<?endif;?>
									<?endif;?>
								</div>
