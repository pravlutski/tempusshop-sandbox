...
<?/*if ($_SERVER['HTTP_HOST'] === 'tempusshop.ru'):?>
	<?
	if (strpos($APPLICATION->GetCurPage(),'/catalog/sale/')<1 and strpos($APPLICATION->GetCurPage(),'/catalog/accessories/')<1) {
	?>
	<div classs="section-text" style="padding-left:30px; padding-top: 40px;">
	<p>В нашем интернет-магазине TempusShop вы можете купить <?=$arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]?>, предварительно ознакомившись с подробными характеристиками, фото и видеообзорами разных моделей. Все изделия в каталоге являются оригиналами, упакованы в фирменную коробку и реализуются с инструкцией на русском языке. </p>
	<h2>Почему приобрести <?=$arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]?> у нас выгоднее, чем в других магазинах?</h2>
	<ol><li><span class="bold">Доступная стоимость.</span> Наша компания закупает большие объемы товара по более низким оптовым ценам, а также снижает издержки за счет отсутствия розничного магазина.
	</li><li><a class="bold" href="/help/delivery/">Быстрая доставка</a>. При заказе на сумму от 3000 рублей действует бесплатная доставка по Москве в пределах МКАД, вы получите <?=$arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]?> уже на следующий день! Возможна доставка день-в-день. В другие регионы часы отправляются ТК и почтой России.
	</li><li><span class="bold">Обширный ассортимент.</span> На нашем сайте вы найдете новинки и самые популярные модели, а также эксклюзивные линейки, доступные по предварительному заказу.
	</li><li><span class="bold">Гарантия подлинности.</span> Все <?=$arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]?> являются оригинальными, имеют наклейки РСТ/EAC и продаются с официальным гарантийным талоном бренда-производителя.</li></ol>
	<p>Наши сотрудники будут рады помочь вам при <a href="/help/">оформлении заказа</a>, <a href="/help/change/">обмене или возврате товаров</a>, настройке часов и в иных вопросах, связанных с эксплуатацией купленного продукта. Обращайтесь!</p>
	</div>
	<? } ?>
<?endif;*/?>
...
//	$title = $start_title . " купить в Минске – подлинные часы Tempus.by";
	...
	*/
	...
	if(count_($arTreeName[0]) > 1) unset($arTreeName[0]);
