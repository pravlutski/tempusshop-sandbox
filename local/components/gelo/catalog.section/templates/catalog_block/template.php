...
<?if($arResult["ITEMS"]):?>
	...
	<?endif;?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<?ob_start();?>
				<div class="sa_block 1" data-fields='<?=Json::encode($arParams["FIELDS"])?>' data-stores='<?=Json::encode($arParams["STORES"])?>' data-user-fields='<?=Json::encode($arParams["USER_FIELDS"])?>'>
                    <?php
                        if(SITE_ID === "s4"){
                            if($arItem["PROPERTIES"]["AVAILABILITY_KZ"]["VALUE_ENUM_ID"] == 1951){
                                $arQuantityData = CMax::GetQuantityArray([
                                    'totalCount' => $arItem["CATALOG_QUANTITY"],
                                    'arItemIDs' => ['ID' => $arItem["ID"]],
                                    'useStoreClick' => (
			...
			<?ob_start();?>
				<div class="item-title">
					<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="<?=$bBigBlock && $bFonImg ? '' : 'dark_link'?> js-notice-block__title option-font-bold font_sm"><span>
            <? if ($arItem['IBLOCK_SECTION_ID'] == '2458') {?>
                <?=$arItem['NAME'];?>
            <?} else {?>
              <?=$elementName;?>
            <?}?>
...
<?//prent($arResult['CUSTOM_RESIZE_OPTIONS']);?>
