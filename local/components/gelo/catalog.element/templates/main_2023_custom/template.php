...
<?php
if (SITE_ID === "s4") {
	if ($arResult["PROPERTY_2887"] == 1951) {
		$arQuantityData = CMax::GetQuantityArray([
			'totalCount' => $arResult["CATALOG_QUANTITY"],
			'arItemIDs' => ['ID' => $arResult["ID"]],
			'useStoreClick' => (
				$arParams["USE_STORE"] == "Y" &&
				$arResult["STORES_COUNT"] &&
				$arResult['CATALOG_TYPE'] != CCatalogProduct::TYPE_SET ? "Y" : "N"
			),
		]);
	}
}
...
<div class="product-info <?= (!$showCustomOffer ? "noffer" : ""); ?> bordered rounded3" id="<?= $arItemIDs["strMainID"]; ?>">
	<script type="text/javascript">
		setViewedProduct(<?= $item_id ?>, <?= CUtil::PhpToJSObject($arViewedData, false) ?>);
	</script>
					...
					<div class="product-detail-gallery__thmb-container text-center">
						<div class="product-detail-gallery__thmb-inner">
							<? if ($countPhoto > 1 || $showCustomOffer) : ?>
								<div class="product-detail-gallery__slider owl-carousel owl-theme thmb<?= ($bVertical ? ' product-detail-gallery__slider--vertical' : ''); ?><?= $countPhoto <= 1 ? ' hidden ' : ' ' ?>" data-size="<?= $countPhoto; ?>" data-plugin-options='{"items": "4", "nav": true, "loop": false, "clickTo": ".product-detail-gallery__slider--big", "dots": false, "autoWidth": true, "margin": 10<? //if($bVertical):
																																																																																																																																																																																																		?>, "mouseDrag": false, "pullDrag": false<? //endif;
																																																																																																																																																																																																																																?><? if ($bMagnifier) : ?>, "magnifier": true<? endif; ?>}' style="max-width:<?= ceil((($countPhoto <= 4 ? $countPhoto : 4) * 70) - 10) ?>px;">
		...
		<div class="right_info">
			<div class="info_item hidden-md hidden-lg">
				...
				<? if ($arResult['HELP_TEXT']) : ?>
					...
					<? if ($arResult["NOT_AVAILABILITY"] === false && $arResult["ITEM_DELIVERY"]) : ?>

						<div class="wrap-delivery" style="">
							<?= $arResult["ITEM_DELIVERY_CITY_HTML"]; ?>
							<?= $arResult["ITEM_DELIVERY_COUNTRY_HTML"]; ?>
						</div>
					<? endif ?>
					...
					<div class="prices_block">
						...
						<? else : ?>
							<div class="cost prices detail prices_block"> 
								<? //dublicate prices_block for send gift?>
								<? if ($arResult["OFFERS"]) : ?>
							...
							<div class="buy_block">
								...
								<? if (!$arResult["OFFERS"]) : ?>
									<div class="counter_wrapp list big clearfix ">
										...
										<?/*
												customAspro 2023
												<div id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block <?=($arAddToBasketData["ACTION"] === "OUT_OF_PRODUCTION" || ($arAddToBasketData["ACTION"] == "ORDER") || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y")  ? "wide" : "");?>">
													<!--noindex-->
										...
										<div id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block <?= (($arAddToBasketData["ACTION"] == "ORDER" /*&& !$arResult["CAN_BUY"]*/) || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y")  ? "wide" : ""); ?>">
											<!--noindex-->
											<?= $arAddToBasketData["HTML"] ?>
											<? if (($arAddToBasketData["ACTION"] == "ORDER" && $arAddToBasketData["CAN_BUY"] != true) || $arAddToBasketData["ACTION"] == "NOTHING") : ?>
												<? if ($arResult["DATE_DISAPPEAR"] !== false) : ?>
													<p class="more_text" style="margin: 10px 0 5px 0;">Последнее поступление: <?= date("d-m-Y", strtotime($arResult["DATE_DISAPPEAR"])) ?></p>
												<? endif;
												// data-event="jqm" data-param-form_id2="ADDTOSUBSCRIBE" data-name2="addtosubscribe"
												?>
												<span class="btn-lg btn btn-default animate-load has-ripple add-to-subscribe" data-event="jqm" data-param-form_id="ADDTOSUBSCRIBE" data-name="addtosubscribe" data-autoload-product_name="<?= $arResult["NAME"] ?>" data-autoload-product_id="<?= $arResult["ID"] ?>"><span>Сообщить о поступлении</span></span>
												<p class="more_text info-add-subscribe">Данной модели сейчас нет в наличии. Однако она может появиться в скором времени. Если вы хотите, чтобы вас известили о ее появлении, оставьте свой email в форме выше. </p>
											<? endif ?>
											<!--/noindex-->
										</div>
									</div>
...
<? if (($arResult['DISPLAY_PROPERTIES'] || $arResult['OFFER_PROP']) && ($iCountProps > $arParams['VISIBLE_PROP_COUNT'])) : ?>
  ...
  font-size: 14px;
  font-style: italic;
  margin-left: 20px;" class="dop_info">
					<p>
						<? if (!empty($arResult['PROPERTIES']['IMPORTER']['VALUE'])) {
							print_r('<u>Импортер:</u> ' . $arResult['PROPERTIES']['IMPORTER']['VALUE']);
						} ?><br>
						<? if (!empty($arResult['PROPERTIES']['PROIZVODITEL']['VALUE'])) {
							print_r('<u>Производитель:</u> ' . $arResult['PROPERTIES']['PROIZVODITEL']['VALUE']);
						} ?>
					</p>
				</div>
			<? } ?>
					...
					<div class="col-md-12 col-sm-6 col-xs-12">
						...
						<div class="file_type clearfix <?= $arFile["TYPE"]; ?>">
							...
							<div class="description">
								<a target="_blank" href="<?= $arFile["SRC"]; ?>" class="dark_link font_sm">Инструкция для <?= $arResult["NAME"]; ?><? // customAspro 2023 =$arFile["DESCRIPTION"];
																																																																?></a>
