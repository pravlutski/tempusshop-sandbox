...
if(strlen($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	...
	//Bitrix\Main\Diag\Debug::writeToFile($arBrand['CATALOG_PAGE_URL'],"","test.txt");
...
$arResult["ITEM_DELIVERY"] = false;

$arCheck = CExchange::controlReserve($arResult["ID"]);
//prent($arResult);
if(SITE_ID == "s1"){
	if($arResult["PROPERTY_282"] == 514 || $arCheck[SITE_ID] === false){
		$arResult["NOT_AVAILABILITY"] = true;
	}else{
		$arResult["NOT_AVAILABILITY"] = false;
	}
} elseif (SITE_ID == "s4"){
	if($arResult["PROPERTY_2887"] == 1952){
		$arResult["NOT_AVAILABILITY"] = true;
	}else{
		$arResult["NOT_AVAILABILITY"] = false;
	}
	...
	if($arResult["PROPERTY_267"] == 494 || $arCheck[SITE_ID] === false){
		$arResult["NOT_AVAILABILITY"] = true;
	}else{
		$arResult["NOT_AVAILABILITY"] = false;
	}
...
if (is_object($cp)){
	$cp->arResult['NOT_AVAILABILITY'] = $arResult["NOT_AVAILABILITY"];
	$cp->SetResultCacheKeys(array('NOT_AVAILABILITY', 'PROPERTIES'));
}
//if($arResult["AVAILABILITY"] === false) $arResult["CATALOG_QUANTITY"] = 0;

if($arResult["CATALOG_QUANTITY"] > 0){
	$arResult["ITEM_DELIVERY"] = getDeliveryDay($arResult["ID"]);
	$arResult["ITEM_DELIVERY_CITY_HTML"] = $arResult["ITEM_DELIVERY_COUNTRY_HTML"] = "";
	if($arResult["ITEM_DELIVERY"]){
		if(SITE_ID == "s1"){
			//prent($arResult["ITEM_DELIVERY"]);
			
			$arResult["ITEM_DELIVERY_CITY_HTML"] .= "<p class='delivery-text'><span>Доставим по Москве</span><b><span style='color: #3c763d;display: inline-block;'>{$arResult["ITEM_DELIVERY"]["CITY"]["day_text"]} {$arResult["ITEM_DELIVERY"]["CITY"]["day_delivery"]}, </span><span style='display: inline-block;color: #e91e63;padding: 0 0 0 5px;'>" . ($arResult["MIN_PRICE"]["DISCOUNT_VALUE"] > 3000 ? "бесплатно" : "300 р.") . "</span></b></p>";
			//prent($arResult["MIN_PRICE"]["DISCOUNT_VALUE"]);
			if($arResult["ITEM_DELIVERY"]["CITY"]["express"] === true && $arResult["MIN_PRICE"]["DISCOUNT_VALUE"] > 3000){
				$arResult["ITEM_DELIVERY_CITY_HTML"] .= "<p class='delivery-text'><span>Экспресс по Москве </span><b><span style='color: #3c763d;display: inline-block;'>сегодня, </span><span style='display: inline-block;color: #e91e63;padding: 0 0 0 5px;'>" . ($arResult["MIN_PRICE"]["DISCOUNT_VALUE"] > 15000 && 1==2 ? "бесплатно" : "500 р.") . "</span></b></p>";
			}
			//if($USER->isAdmin()){
				$add_text =	"<span class='animate-load dotted' data-event='jqm' data-param-form_id='delivery' data-name='delivery' data-param-product_id='{$arResult["ID"]}' style='display: inline-block;    padding: 0;color: #e91e63;margin: 0 0 0 5px;'> рассчитать</span>";
			//}
			$arResult["ITEM_DELIVERY_COUNTRY_HTML"] .= "<p class='delivery-text'><span>Отправим по России</span><b><span style='color: #3c763d;    display: inline-block;'>{$arResult["ITEM_DELIVERY"]["COUNTRY"]["day_text"]} {$arResult["ITEM_DELIVERY"]["COUNTRY"]["day_delivery"]},</span>{$add_text}</b></p>";
		}elseif(SITE_ID == "s2"){
			$add_text = "<span style='display: inline-block;color: #3c763d;padding: 0 0 0 0px;'>, </span>";
			if($arResult["MIN_PRICE"]["DISCOUNT_VALUE"] > 50){
				$add_text .= "<span style='display: inline-block;color: #e91e63;padding: 0 0 0 5px;'>бесплатно</span>";
			}else{
				$add_text .= "<span style='display: inline-block;color: #e91e63;padding: 0 0 0 5px;'>5 руб.</span>";
			}
			$arResult["ITEM_DELIVERY_CITY_HTML"] .= "<p class='delivery-text'><a href='https://www.tempus.by/contacts/stores/129460/' target='_blank' style='display: block;'><span style='border-bottom: 1px solid #e6e6e6;color: #777;display: inline-block;margin: 0 0 0 30px;padding: 0;'>Самовывоз</span></a><b><span style='display: inline-block;color: #3c763d;'>{$arResult["ITEM_DELIVERY"]["CITY"]["day_text"]} {$arResult["ITEM_DELIVERY"]["CITY"]["day_delivery"]}, </span><span style='display: inline-block;color: #e91e63;padding: 0 0 0 5px;'>бесплатно</span></b></p>";
			$arResult["ITEM_DELIVERY_CITY_HTML"] .= "<p class='delivery-text'><span style=''>Доставим по Минску</span><b><span style='display: inline-block;color: #3c763d;'>{$arResult["ITEM_DELIVERY"]["CITY"]["day_text"]} {$arResult["ITEM_DELIVERY"]["CITY"]["day_delivery"]}</span>{$add_text}</b></p>";
			
			$arResult["ITEM_DELIVERY_COUNTRY_HTML"] .= "<p class='delivery-text'><span style=''>Отправим по Беларуси</span><b><span style='display: inline-block;color: #3c763d;'>{$arResult["ITEM_DELIVERY"]["COUNTRY"]["day_text"]} {$arResult["ITEM_DELIVERY"]["COUNTRY"]["day_delivery"]}</span>{$add_text}</b></p>";
		}
	}
...
$arResult["DATE_DISAPPEAR"] = false;
if($arResult["CATALOG_QUANTITY"] <= 0 && strlen($arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) > 0){
	if (CModule::IncludeModule('panel.manager')){
		$res = (new CPanelPricelist)->getDateUpdate($arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]);
		if (!empty($res["DATE_DISAPPEAR"]) && $res["DATE_DISAPPEAR"] !== '0000-00-00' && $res["DATE_DISAPPEAR"] !== '0000-00-00 00:00:00')
			$arResult["DATE_DISAPPEAR"] = $res["DATE_DISAPPEAR"];
	}
}
