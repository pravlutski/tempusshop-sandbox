...
?>
<?/*
	// customAspro 2023 . Блок "Вам также может понравиться" у не доступных товаров
	$ID = $arResult["ID"];
...
<?if($arResult["ID"]):?>
	...
	//$arResult["AVAILABILITY"] = $arCheck[SITE_ID];
	//prent($arResult);
	//$arCheck = CExchange::controlReserve($arResult["ID"]);
	//prent($arResult);
	?>
	...
	<?//ob_start()?>
	<?if($arResult["NOT_AVAILABILITY"] === true) {?>
		<div id="to-goods" class="ordered-block <?=$code?>" <?if (SITE_ID == "s3"){?>style='display:none'<?}?>>
			<div class="ordered-block__title option-font-bold font_lg"><?=$arParams["DETAIL_ASSOCIATED_TITLE"];?></div>
			<div class="assoc-block js-load-block loader_circle test" data-block="assoc" data-file="<?=$APPLICATION->GetCurPage()?>">
				<div class="stub"></div>
	...
	<?};?>
	<?//$html=ob_get_clean();?>
	<?//$APPLICATION->AddViewContent('PRODUCT_RECOMMENT', $html, 100);?>
									...
									<div class="tab-pane <?=$value;?> <?=$arParams['REVIEWS_VIEW']?> <?=(!($iTab++) ? 'active' : '')?>" id="reviews">
										...
										<div id="reviews_content" class="<?=$arParams['REVIEWS_VIEW'] == 'EXTENDED' ? '' : 'bordered rounded3'?>">
											...
											if(1==1):?>
												...
												<?$APPLICATION->IncludeComponent("op:reviews.element", "template1", Array(
	...
	false
		...
		<?//goods?>
		<?elseif($code == 'goods'):?>
		<?//добавлено 21.04.2023 начало?>
		<?if($arResult["NOT_AVAILABILITY"] === false) :?>
			<div id="to-goods" class="ordered-block 2 <?=$code?>" <?if (SITE_ID == "s3"){?>style='display:none'<?}?>>
				<div class="ordered-block__title option-font-bold font_lg"><?=$arParams["DETAIL_ASSOCIATED_TITLE"];?></div>
				<?include($_SERVER['DOCUMENT_ROOT'].SITE_DIR.'/include/detail.linked_products_block.php');?>
				<?//edit?>
				<div class="assoc-block js-load-block loader_circle test" data-block="assoc" data-file="<?=$APPLICATION->GetCurPage()?>" style="display:none">
					<div class="stub"></div>
					<?CMax::checkRestartBuffer(true, 'assoc');?>
						<?if(CMax::checkAjaxRequest()):?>
							<?//$APPLICATION->ShowAjaxHead();?>
			...
			<?if($arParams['DETAIL_LINKED_GOODS_TABS'] != 'N'):?>
				...
				<div class="ordered-block <?=$code?>">
					...
					<?if($templateData['ASSOCIATED'] && $templateData['EXPANDABLES']):?>
						...
						<?endif;?>
					<?endif;?>
					<?if($templateData['ASSOCIATED'] && 1==2):?>
						<?
						$bShowAssociatedTab = \Aspro\Functions\CAsproMax::checkAvailable($templateData['ASSOCIATED'], array('REGION'), $arParams);
						?>
						<?if($bShowAssociatedTab):?>
					...
					<?if($templateData['EXPANDABLES'] && 1==2):?>
						<?
						$bShowExpandablesTab = \Aspro\Functions\CAsproMax::checkAvailable($templateData['EXPANDABLES'], array('REGION'), $arParams);
						?>
						<?if($bShowExpandablesTab):?>
