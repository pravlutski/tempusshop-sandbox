	...
	<div class="right_info">
		<div class="info_item hidden-md hidden-lg">
						...
						<div class="js-prices-in-side">
							<div class="buy_block">
								...
								<?if($arAddToBasketData["ACTION"] == "SUBSCRIBE"):?>
									<?if($arResult["DATE_DISAPPEAR"] !== false):?>
									<p style="margin: 0 0 5px 0;">Последнее поступление: <?=date("d-m-Y", strtotime($arResult["DATE_DISAPPEAR"]))?></p>
									<?endif?>
									<span class="big_btn transparent add-to-subscribe button" rel="nofollow" data-item="<?=$arResult["ID"]?>" onclick="addSubscribeProduct('<?=$arResult["ID"]?>', '<?=$arResult["IBLOCK_ID"]?>', this)"><i></i><span>Сообщить о поступлении</span></span>
									<p class="info-add-subscribe">Данной модели сейчас нет в наличии. Однако она может появиться в скором времени. Если вы хотите, чтобы вас известили о ее появлении, оставьте свой номер телефона в форме выше. </p>
								<?else:?>asd
			...
			<?$this->SetViewTarget('PRODUCT_SIDE_INFO', 900);?>
				<?if($arResult['BRAND_ITEM']):?>
					<div class="brand-detail t-detail">
...
<?if(!$arResult['DETAIL_TEXT']):?>
	...
	<?$this->SetViewTarget('PRODUCT_CHR_OP');?>
		<div class="content" itemprop="description-op">
			<div class="add-text-item">
				<p class="title">Описание</p>
					<?
					
					//prent($arResult["PROPERTIES"]["BRAND"]);
					?>
				<p>Предлагаем вашему вниманию <?/*if($arResult["SECTION"]["PATH"][0]["CODE"] == "accessories"):?>оригинальную<?else:*/?>оригинальные<?//endif?> <?=modifyTextItem($arResult["PROPERTIES"]["SEX"]["VALUE"], $arResult["SECTION"]["PATH"], "lower", 1)?> 
				<?/*if(count($arResult["SECTION"]["PATH"]) == 2):?>
					<a href="<?=$arResult["SECTION"]["PATH"][1]["SECTION_PAGE_URL"]?>"><?=$arResult["PROPERTIES"]["BRAND"]["PROPERTY_VALUE_ID"][$arResult["PROPERTIES"]["BRAND"]["VALUE"]]["NAME"]?></a>
				<?endif*/?> 
				<?if(count($arResult["SECTION"]["PATH"]) == 3):?>
				<?
				//prent($arResult);
				//prent($arResult["PROPERTIES"]["BRAND"]["VALUE"]);
				?>
					<a href="<?=$arResult["SECTION"]["PATH"][1]["SECTION_PAGE_URL"]?>"><?=$arResult["SECTION"]["PATH"][1]["NAME"]?></a>
				<?endif?> 
				<a href="<?=$arResult["SECTION"]["SECTION_PAGE_URL"]?>"><?=$arResult["SECTION"]["NAME"]?></a> <?=$arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]?> <?if($arResult["PROPERTIES"]["BRANDCONTRY"]["VALUE"]):?>(<?=$arResult["PROPERTIES"]["BRANDCONTRY"]["VALUE"]?>)<?endif?>.</p>
				<ul>
					<?if($arResult["PROPERTIES"]["MECHANISM"]["VALUE"]):?><li>Тип механизма - <?=$arResult["PROPERTIES"]["MECHANISM"]["VALUE"]?>;</li><?endif?>
					<?if($arResult["PROPERTIES"]["GLASS"]["VALUE"]):?><li><?=$arResult["PROPERTIES"]["GLASS"]["VALUE"]?> стекло;</li><?endif?>
					<?if($arResult["PROPERTIES"]["DIAMETER"]["VALUE"]):?><li>Размер корпуса - <?=$arResult["PROPERTIES"]["DIAMETER"]["VALUE"]?> мм;</li><?endif?>
					<?if($arResult["PROPERTIES"]["CASE"]["VALUE"]):?><li>Материал корпуса - <?=$arResult["PROPERTIES"]["CASE"]["VALUE"]?>.</li><?endif?>
				</ul>
				<p>В нашем интернет-магазине вы сможете ознакомиться с характеристиками часов, отзывами покупателей, фото- и видеообзорами. Купить <?=modifyTextItem($arResult["PROPERTIES"]["BRANDCONTRY"]["VALUE"], $arResult["SECTION"]["PATH"], "lower", 2)?> <?=$arResult["PROPERTIES"]["BRAND"]["PROPERTY_VALUE_ID"][$arResult["PROPERTIES"]["BRAND"]["VALUE"]]["NAME"]?> <?=$arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]?> вы можете по цене <?=$arResult["PRICES"]["BASE"]["PRINT_DISCOUNT_VALUE"]?> с быстрой доставкой в любой город России.
				<?if($arResult["PROPERTIES"]["WARRANTY"]["VALUE"]):?><p>Официальная гарантия на данную модель – <?=$arResult["PROPERTIES"]["WARRANTY"]["VALUE"]?>.</p><?endif?>
				<hr>
			</div>
		</div>
	<?$this->EndViewTarget();?>
	...
	<?$this->SetViewTarget('PRODUCT_CHR_OP');?>
		<div class="content" itemprop="description-op">
			<div class="add-text-item">
				...
				<p>Предлагаем вашему вниманию <?/*if($arResult["SECTION"]["PATH"][0]["CODE"] == "accessories"):?>оригинальную<?else:*/?>оригинальные<?//endif?> <?=modifyTextItem($arResult["PROPERTIES"]["SEX"]["VALUE"], "lower", $arResult["SECTION"]["PATH"][0]["CODE"])?> 
				<?/*if(count($arResult["SECTION"]["PATH"]) == 2):?>
					<a href="<?=$arResult["SECTION"]["PATH"][1]["SECTION_PAGE_URL"]?>"><?=$arResult["PROPERTIES"]["BRAND"]["PROPERTY_VALUE_ID"][$arResult["PROPERTIES"]["BRAND"]["VALUE"]]["NAME"]?></a>
				<?endif*/?> 
				<?if(count($arResult["SECTION"]["PATH"]) == 3):?>
				...
				<p>В нашем интернет-магазине вы сможете ознакомиться с характеристиками часов, отзывами покупателей, фото- и видеообзорами. Купить <?=modifyTextItem($arResult["PROPERTIES"]["BRANDCONTRY"]["VALUE"], "lower", $arResult["SECTION"]["PATH"][0]["CODE"])?> <?=$arResult["PROPERTIES"]["BRAND"]["PROPERTY_VALUE_ID"][$arResult["PROPERTIES"]["BRAND"]["VALUE"]]["NAME"]?> <?=$arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]?> вы можете по цене <?=$arResult["PRICES"]["BASE"]["PRINT_DISCOUNT_VALUE"]?> с быстрой доставкой в любой город России.
				<?if($arResult["PROPERTIES"]["WARRANTY"]["VALUE"]):?><p>Официальная гарантия на данную модель – <?=$arResult["PROPERTIES"]["WARRANTY"]["VALUE"]?>.</p><?endif?>
				<hr>
			</div>
		</div>
						...
						<div class="col-md-4 col-sm-6">
							...
							<div class="file_type clearfix <?=$arFile["TYPE"];?>">
								...
								<div class="description">
									<a target="_blank" href="<?=$arFile["SRC"];?>" class="dark_link font_sm">Инструкция для <?=$arResult["NAME"];?><?//=$arFile["DESCRIPTION"];?></a>
...
<?
//op коммент 21.07.2020
if (1==2 && $arResult['CATALOG'] && $arParams['USE_GIFTS_DETAIL'] == 'Y' && \Bitrix\Main\ModuleManager::isModuleInstalled("sale"))
{
...
if (1==2 && $arResult['CATALOG'] && $arParams['USE_GIFTS_MAIN_PR_SECTION_LIST'] == 'Y' && \Bitrix\Main\ModuleManager::isModuleInstalled("sale"))
{
	$APPLICATION->IncludeComponent(
