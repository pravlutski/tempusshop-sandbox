...
?>
<?
	// Блок "Вам также может понравиться" у не доступных товаров
	$ID = $arResult["ID"];
	$ar_res = CCatalogProduct::GetByID($ID);

	if(($ar_res["QUANTITY"] <= 0) && ($ar_res["AVAILABLE"] == 'N')) {?>
		<div id="to-goods" class="ordered-block <?=$code?>" <?if (SITE_ID == "s3"){?>style='display:none'<?}?>>
			<div class="ordered-block__title option-font-bold font_lg"><?=$arParams["DETAIL_ASSOCIATED_TITLE"];?></div>
			<div class="assoc-block js-load-block loader_circle test" data-block="assoc" data-file="<?=$APPLICATION->GetCurPage()?>">
				<div class="stub"></div>
...
<?if($arResult["ID"]):?>
	...
	<?foreach($arBlockOrder as $code):?>
						...
						<div class="tabs arrow_scroll">
							<ul class="nav nav-tabs font_upper_md">
								...
								<?foreach($arTabOrder as $value):?>
									<?//show desc block?>
									<?if($value == "desc"):?>
										<?if($bShowDetailTextTab || ($arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB" && $bShowPropsTab) || 1==1):?>
									...
									<?if($value == "reviews"):?>
										<?if($bShowReviewsTab):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>">
												<a href="#reviews" data-toggle="tab"><?=$arParams["TAB_REVIEW_NAME"];?><?$APPLICATION->ShowViewContent('PRODUCT_REVIWS_COUNT_INFO')?> <span id="product_cnt_reviews"></span></a></li>
										<?endif;?>
									<?endif;?>
									<?//show video block?>
									<?if($value == "video"):?>
										...
										<?if(!$bShowDetailText):?>
											<?if($i == 1):?>
												<div class="ordered-block__title option-font-bold font_lg">
													<?=Loc::getMessage("DESCRIPTION_TAB");?>
												</div>
											<?endif;?>
											<?if($USER->isAdmin()):?>
											<?$APPLICATION->ShowViewContent('PRODUCT_CHR_OP')?>
											<?endif?>
										<?endif;?>
										<?if($bShowDetailText):?>
											<?if($i == 1):?>
								...
								<?if($bShowReviewsTab):?>
									<div class="tab-pane <?=$value;?> <?=$arParams['REVIEWS_VIEW']?> <?=(!($iTab++) ? 'active' : '')?>" id="reviews">
										...
										<div id="reviews_content" class="<?=$arParams['REVIEWS_VIEW'] == 'EXTENDED' ? '' : 'bordered rounded3'?>">
											<?
											//название товара для отзывов
											$GLOBALS["TOVNAME"]=$arResult["NAME"];						
											if(1==1):?>
												<? //разметка пропадала из-за кэша, убрал область												//Bitrix\Main\Page\Frame::getInstance()->startDynamicWithID("area");?>
												<?$APPLICATION->IncludeComponent("op:reviews.element", "", array("PRODUCT_ID" => $arResult["ID"]), false);?>
												<?//Bitrix\Main\Page\Frame::getInstance()->finishDynamicWithID("area", "");?>
											<?elseif($templateData["YM_ELEMENT_ID"]):?>
												<?$APPLICATION->IncludeComponent(
		...
		<?//show reviews block?>
		<?elseif($code == "reviews" && $bShowReviewsTab && 1==2):?>
		...
		<?elseif($code == 'goods'):?>
			<?if($arParams['DETAIL_LINKED_GOODS_TABS'] != 'N'):?>
				...
				<div id="to-goods" class="ordered-block <?=$code?>" <?if ((SITE_ID == "s3") || (($ar_res["QUANTITY"] <= 0) && ($ar_res["AVAILABLE"] == N))){?>style='display:none'<?}?>>
					<?$bNavTabs = false;?>
...
<?
//{=this.Name} купить часы в Москве в интернет-магазине
global $APPLICATION;
$arTreeName = array();
$isAccessories = false;
if(count($arResult["SECTION"]["PATH"]) > 0){
	foreach($arResult["SECTION"]["PATH"] as $key => $arItem){
		if($key > 0)
			$arTreeName[] = $arItem["NAME"];
		elseif($arItem["CODE"] == "accessories")
			$isAccessories = true;
	}
}
//prent($arResult);
$objRes = CIBlockElement::GetList(array(), array('IBLOCK_ID' => CProSet::IB_CATALOG, 'ID' => $arResult["ID"]), false, false, array('PROPERTY_CML2_ARTICLE'));
if ($res = $objRes->GetNext()){
	if(strlen($res["PROPERTY_CML2_ARTICLE_VALUE"]) > 0)
		$arTreeName[] = $res["PROPERTY_CML2_ARTICLE_VALUE"];
	else
		$arTreeName[] = $arResult["NAME"];
}else{
	$arTreeName[] = $arResult["NAME"];
}
...
if(SITE_ID == "s2"){
	...
	$title = "Часы {$arResult["NAME"]} купить в интернет-магазине Tempus.by по доступной цене. Большой выбор подлинных часов, доставка по всей Беларуси.";

	$description = "Часы {$arResult["NAME"]} купить в интернет-магазине Tempus.by по доступной цене. Большой выбор подлинных часов, доставка по всей Беларуси.";

	$h1 = $arResult["NAME"];
	
	$pageadr = '...' . $_SERVER["REQUEST_URI"];	
	//Наручные часы
	if (strpos($pageadr,'/catalog/watches/')>1) {
		$title = $h1." — купить наручные часы со скидкой 5% в Tempus";

		$description = "Предоставляем лучшую цену на &#8986; ".$h1." и возможность купить и получить товар в Минске на следующий день! &#128666; Быстрая доставка в любую точку Беларуси. Все наручные часы в интернет-магазине являются оригинальными, ".$h1." упакованы в фирменную коробку и имеют гарантию производителя. Ищите подробные описания, фото и &#128249; видеообзоры популярных моделей на сайте!";


	}	
	// другие часы
	else if (strpos($pageadr,'/catalog/wall_clock/')>1 or strpos($pageadr,'/catalog/table_clock/')>1 or strpos($pageadr,'/catalog/pocket_watch/')>1) {
		
		$title = $h1." – купить часы в интернет-магазине Tempus";
		$description = "Предоставляем лучшую цену на &#8986; ".$h1." и возможность купить и получить товар в Минске на следующий день! &#128666; Быстрая доставка в любую точку Беларуси. Все часы в интернет-магазине являются оригинальными, ".$h1." упакованы в фирменную коробку и имеют гарантию производителя. Ищите подробные описания и фото популярных моделей на сайте!";

		
	}	
	else if (strpos($pageadr,'/catalog/accessories/')>1) {	
		$title = $h1." – купить оригинал | Каталог интернет-магазина Tempus";
		$description = "Предоставляем лучшую цену на &#8986; ".$h1.", возможность купить и получить товар в Минске на следующий день! &#128666; Быстрая доставка по Беларуси. Заказывайте и носите с удовольствием!";	
	}	
	...
	//$GLOBALS['h1tag']=$h1;
	$GLOBALS['titletag']=$title;
	$GLOBALS['descriptiontag']=$description;

}else{

	$title = ($isAccessories === false ? "Наручные часы " : "");
	$title .= implode(" ", $arTreeName) . " - купить в Москве";

	//Оригинальные наручные часы Casio EQB-1000D-1A по цене производителя в официальном интернет-магазине. Характеристики. Фото и видео обзоры. Инструкция. Отзывы покупателей. Доставка по Москве и России.
	
	//$description = ($isAccessories === false ? "Оригинальные наручные часы " : "Оригинальная ");
	//$description .= implode(" ", $arTreeName) . " по цене производителя в официальном интернет-магазине. Характеристики. Фото и видео обзоры. Инструкция. Отзывы покупателей. Доставка по Москве и России.";
	//$description = "Купить часы {$arResult["NAME"]} в интернет-магазине Tempusshop. Широкий выбор подлинных часов от официальных дистрибьютеров, с доставкой по России.";
	
	//Продажа наручных часов %бренд% %коллекция% %модель% по низкой цене в официальном интернет-магазине. Характеристики. Фото и видео обзоры. Отзывы покупателей. Доставка по России.
	$description = ($isAccessories === false ? "Продажа наручных часов " : "Продажа аксессуаров ");
	$description .= implode(" ", $arTreeName) . " по низкой цене в официальном интернет-магазине. Характеристики. Фото и видео обзоры. Отзывы покупателей. Доставка по России.";
	
	//Наручные часы Casio Edifice EQB-1000D-1A
	$h1 = ($isAccessories === false ? "Наручные часы " : "");
...
*/
GLOBAL $lastModified;
$lastModified = strtotime(getProductTimestamp($arParams["ELEMENT_CODE"]));
//prent($arParams);
?>
