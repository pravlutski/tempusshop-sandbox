...
/*************************************************************************
			Work with cache
*************************************************************************/
$cache_id = md5(SITE_ID . "|" . getProductTimestamp($arParams["ELEMENT_CODE"]) . serialize(getDeliveryDayByCode($arParams["ELEMENT_CODE"])));
$cachePath = "/".SITE_ID."/gelo/catalog.element/" . $arParams["ELEMENT_CODE"];
...
if($this->startResultCache($arParams["CACHE_TIME"], $cache_id, $cachePath)){
	...
	if($arParams["ELEMENT_ID"] > 0)
	{
		...
		if ($obElement = $rsElement->GetNextElement())
		{
			...
			$CACHE_MANAGER->RegisterTag('product_' . $arParams["ELEMENT_ID"]);
