(function(BX, $) {
	...
	var constructor = Input.DependField = Plugin.Base.extend({
		...
		toggleAdditionView: function(isMatch) {
			var prev = this.getElement('prevAddition', this.$el, 'prev');
			var next = this.getElement('nextAddition', this.$el, 'next');
		...
		getField: function(selector) {
			...
			if (selector.substring(0, 1) === '#') {
				...
				result = $(`[name="${selector.substring(1)}"]`);
			} else {
		...
		getFormField: function(name) {
			...
			} else if (isForm && form[0][nameMultipleSecond] != null) {
				result = $(form[0][nameMultipleSecond]);
			} else {
				variants = [
					'[data-name="' + name + '"]',
					'[data-name^="[' + name + ']"]',
