...
class AgentSkeleton extends Market\Reference\Agent\Base
{
	...
	protected static function canRepeat($exception, $errorCount)
	{
		return (
			...
			&& $errorCount < static::getErrorRepeatLimit()
		);
	}
	...
	protected static function actionError($message, $arguments = null)
	{
		...
		{
			if (
				!($message instanceof Main\SystemException)
				&& ($message instanceof \Exception || $message instanceof \Throwable)
			)
			{
				Main\Application::getInstance()->getExceptionHandler()->writeToLog($message);
			}
	...
	protected static function getSetup($setupId)
	{
		...
		if (!$setup->isActive())
		{
			throw new Main\ObjectPropertyException(sprintf('setup %s is inactive', $setupId));
		}
