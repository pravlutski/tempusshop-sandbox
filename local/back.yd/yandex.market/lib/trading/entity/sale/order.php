...
class Order extends Market\Trading\Entity\Reference\Order
{
	...
	public function getTradingInfo()
	{
		return $this->getTradingInfoFromTradeBinding() ?: $this->getSetupIdFromXmlId();
	}

	protected function getTradingInfoFromTradeBinding()
	{
		if (!method_exists($this->internalOrder, 'getTradeBindingCollection')) { return null; }

		/** @var Sale\TradeBindingEntity $binding */
		foreach ($this->internalOrder->getTradeBindingCollection() as $binding)
		{
			$platformId = (int)$binding->getField('TRADING_PLATFORM_ID');

			if (!Internals\Platform::isOur($platformId)) { continue; }

			$params = $binding->getField('PARAMS');

			return [
				'PLATFORM_ID' => $platformId, // old behavior
				'TRADING_PLATFORM_ID' => $platformId,
				'ORDER_ID' => (string)$binding->getField('EXTERNAL_ORDER_ID'),
				'SETUP_ID' => is_array($params) && !empty($params['SETUP_ID']) ? (int)$params['SETUP_ID'] : null,
			];
		}
	...
	protected function getSetupIdFromXmlId()
	{
		return Platform::parseOrderXmlId((string)$this->internalOrder->getField('XML_ID'));
	}
