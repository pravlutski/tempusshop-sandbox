		...
		TradingEntity\Reference\OutletInvertible
{
	...
	public function getOutlets(TradingEntity\Reference\Order $order, $deliveryId, Api\Model\Region $region)
	{
		...
		$outlets = $this->extractOutlets($outlets, $sdekCity['NAME']);

		if (empty($outlets)) { return []; }

		$prefix = $this->getPrefix();
		$codes = array_keys($outlets);
	...
	protected function extractOutlets(array $outlets, $cityName)
	{
		$suburbs = $this->allSuburbs();
		$result = !empty($outlets[$cityName]) && is_array($outlets[$cityName]) ? $outlets[$cityName] : [];

		if (!empty($suburbs[$cityName]))
		{
			foreach ($suburbs[$cityName] as $suburb)
			{
				if (!isset($outlets[$suburb]) || !is_array($outlets[$suburb])) { continue; }

				$result += $outlets[$suburb];
			}
		}
	...
	protected function allSuburbs()
	{
		$rows = explode(PHP_EOL, trim(self::getMessage('SUBURB')));
		$result = [];
		$mainName = null;
		...
		foreach ($rows as $i => $row)
		{
			...
			{
				$result[$mainName] = explode(',', trim($row));
			}
		}
