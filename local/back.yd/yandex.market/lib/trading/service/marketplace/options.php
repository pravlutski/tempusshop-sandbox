...
class Options extends TradingService\Common\Options
{
	...
	public function takeChanges(TradingService\Reference\Options\Skeleton $previous)
	{
		...
		$this->takeYandexModeChanges($previous);
	}
	...
	protected function compareStoreChanges(Options $previous)
	{
		...
		{
			$changed = $previous->getValue('WAREHOUSE_STORE_FIELD') !== $this->getValue('WAREHOUSE_STORE_FIELD');
		}
		else
		{
	...
	protected function takeYandexModeChanges(Options $previous)
	{
		try
		{
			$mode = $this->getValue('YANDEX_MODE');

			if (!$mode || $mode === $previous->getValue('YANDEX_MODE')) { return; }
	...
	protected function getCommonFields(TradingEntity\Reference\Environment $environment, $siteId)
	{
		...
		$result['YANDEX_MODE'] = [
			'TYPE' => 'enumeration',
			'NAME' => static::getLang('TRADING_SERVICE_MARKETPLACE_OPTION_YANDEX_MODE'),
			'HIDDEN' => Market\Config::getOption('trading_yandex_mode') === 'Y' || $this->getYandexMode() !== static::YANDEX_MODE_PULL ? 'N' : 'Y',
			'MANDATORY' => 'Y',
			'DESCRIPTION' => static::getLang('TRADING_SERVICE_MARKETPLACE_OPTION_YANDEX_MODE_DESCRIPTION'),
			'VALUES' => [
				[
					'ID' => static::YANDEX_MODE_PULL,
					'VALUE' => static::getLang('TRADING_SERVICE_MARKETPLACE_OPTION_YANDEX_MODE_PULL'),
				],
				[
					'ID' => static::YANDEX_MODE_PUSH,
					'VALUE' => static::getLang('TRADING_SERVICE_MARKETPLACE_OPTION_YANDEX_MODE_PUSH'),
				],
			],
			'SORT' => 1001,
			'SETTINGS' => [
	...
	{
		$result = parent::getIncomingRequestFields($environment, $siteId);

		foreach (['YANDEX_TOKEN', 'YANDEX_INCOMING_URL'] as $name)
		{
			$result[$name]['DEPEND'] = [
				'LOGIC' => 'OR',
				'YANDEX_TOKEN' => [
