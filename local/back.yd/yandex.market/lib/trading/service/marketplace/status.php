...
class Status extends TradingService\Common\Status
{
	...
	public function canMove($status)
	{
		return ($status === static::STATUS_PROCESSING);
	}

	public function allowedStatuses($status, $substatus = null, $isCancelRequested = false)
	{
		if (!$this->isProcessing($status)) { return []; }
		...
		$substatusOrder = array_diff_key($this->getSubStatusProcessOrder(), [
			static::STATE_SHIPPED => true,
		]);

		if (!isset($substatusOrder[$substatus]))
		{
			return array_keys($substatusOrder);
		}
		...
		foreach ($substatusOrder as $processSubstatus => $processOrder)
		{
			if (
				...
				|| ($processOrder > $currentOrder && $processSubstatus === static::STATE_SHOP_FAILED)
			)
			{
