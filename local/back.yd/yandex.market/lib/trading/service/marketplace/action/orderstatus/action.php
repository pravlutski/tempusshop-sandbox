...
class Action extends TradingService\Common\Action\OrderStatus\Action
{
	...
	protected function statusConfiguredAction($status)
	{
		...
		if (empty($result) && $status === TradingService\Marketplace\Status::STATUS_PROCESSING . '_POSTPAID')
		{
	...
	{
		if (!$this->provider->getOptions()->includeBasketSubsidy()) { return []; }

		$subsidyStatus = [];
		$prepaidMappings = array_merge(
		...
		foreach ($prepaidMappings as $configuredPrepaid)
		{
			...
			$subsidyStatus = (array)$meaningfulMap[Market\Data\Trading\MeaningfulStatus::SUBSIDY];
			break;
		}
	...
	protected function finalizeTrackReturn()
	{
		if (
			...
			|| count($this->provider->getOptions()->getStatusIn(TradingService\Marketplace\Status::VIRTUAL_RETURN)) === 0
		)
		{
			return;
		}
