...
class Action extends TradingService\Reference\Action\DataAction
{
	...
	protected function getOrderRow(Market\Api\Model\Order $order, TradingEntity\Reference\Order $bitrixOrder = null)
	{
		...
		$result = [
			...
			'STATUS_READY' => $bitrixOrder !== null && $this->provider->getStatus()->canMove($order->getStatus()),
			'STATUS_ALLOW' => $bitrixOrder !== null
				? $this->provider->getStatus()->allowedStatuses($order->getStatus(), $order->getSubStatus(), $order->isCancelRequested())
				: [],
		...
		{
			$result['TOTAL'] = $order->getItemsTotal() + $order->getSubsidies()->getItemsSum();
			$result['SUBSIDY'] = $order->getSubsidies()->getItemsSum();
		}
