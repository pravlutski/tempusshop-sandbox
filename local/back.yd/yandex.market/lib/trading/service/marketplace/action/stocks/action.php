	...
	protected function collectSku($productAmounts, $offerMap)
	{
		...
		foreach ($offersAmountMap as $offerId => $productAmount)
		{
			...
			else if (isset($productAmount['QUANTITY']))
			{
				...
				$sql = "SELECT count FROM ci_price WHERE bitrix_id = '".$productAmount['ID']."' AND active_ya = 'Y'";
				...
				$sql_r = "SELECT RESERVED FROM ci_reserved WHERE PRODUCT_ID  = '".$productAmount['ID']."'";
				$res = $connection->query($sql_r);
				while ($r = $res->fetch()) {
					$reserv += intval($r);
				}
				if (($count - $reserv) <= 0) {
