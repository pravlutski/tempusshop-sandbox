...
use Yandex\Market\Trading\Facade\Action as FacadeAction;
	...
	implements TradingService\Reference\Action\HasActivity
{
	...
	protected function fixStatus(Main\Result $sendResult, $orderId, $state)
	{
		...
		return (
			$this->fixStatusByCis($sendResult, $orderId, $state)
			|| $this->fixStatusBySubmitStack($sendResult, $orderId, $state)
		);
	}

	protected function fixStatusByCis(Main\Result $sendResult, $orderId, $state)
	{
		if (!$this->hasSendResultCisError($sendResult) || $this->hasManualInstances()) { return false; }

		$facade = new FacadeAction\SendOrderBoxes($this->getOrder());
		$submitted = $facade->submit();

		if (!$submitted->isSuccess() || $submitted->hasWarnings()) { return false; }

		$resend = $this->sendStatus($orderId, $state);

		return $resend->isSuccess();
	}

	protected function hasSendResultCisError(Main\Result $sendResult)
	{
		foreach ($sendResult->getErrors() as $error)
		{
			if (
				mb_strpos($error->getMessage(), 'Invalid CIS format') !== false
				|| mb_strpos($error->getMessage(), 'No CIS identifiers are present') !== false
			)
			{
				return true;
			}
