		...
		foreach ($statusOrder as $processSubstatus => $processOrder)
		{
			if (
				...
				|| (
					...
					&& $processSubstatus === static::STATUS_CANCELLED
				)
			)
			{
				$result[] = $processSubstatus;

				if ($processSubstatus === static::STATUS_PICKUP)
				{
					$gap = 2;
				}
			}
