	...
	implements TradingService\Reference\Action\HasActivity
{
	...
	protected function deliveryId()
	{
		...
		return (
			...
			?: $this->bitrixOrder->getDeliveryId()
		);
	}
	...
	protected function getPropertyValue($propertyName)
	{
		...
		{
			$result = $this->externalOrder->getItemsTotal() + $this->externalOrder->getDeliveryTotal();
		}
	...
	protected function getBasketSummaryValues()
	{
		...
		$deliveryTotal = $this->externalOrder->getDeliveryTotal();
		$subsidyTotal = $this->externalOrder->getSubsidies()->getSum();

		return array_filter([
			'ITEMS_TOTAL' => Market\Data\Currency::format($itemsTotal, $currency),
			'SUBSIDY_TOTAL' => $subsidyTotal > 0
				? Market\Data\Currency::format($subsidyTotal, $currency)
				: null,
			'DELIVERY' => Market\Data\Currency::format($deliveryTotal, $currency),
			'TOTAL' => Market\Data\Currency::format($itemsTotal + $subsidyTotal + $deliveryTotal, $currency),
		]);
	}
