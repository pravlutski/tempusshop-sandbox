...
class IblockSection implements Strategy
{
	...
	public function getFields()
	{
		return [
			...
			'UTM_TEMPLATE' => [
				'TYPE' => 'string',
				'NAME' => self::getMessage('UTM_TEMPLATE'),
				'HELP_MESSAGE' => self::getMessage('UTM_TEMPLATE_HELP'),
				'SETTINGS' => [
					'SIZE' => 60,
					'ROWS' => 2,
				],
	...
	protected function sectionRows($iblockId, array $sectionIds)
	{
		...
		if ($this->needUtmCompile())
		{
			$utmSourceSelect = $this->utmTemplate()->getSourceSelect();
			$utmSectionSelect = (array)$utmSourceSelect['section'];
		}
		...
		while ($section = $query->GetNext())
		{
			...
			foreach ($fieldsMap as $target => $field)
			{
				...
				{
					$exportSection[$target] = null;
					continue;
				}

				$exportSection[$target] = $this->displayValue($iblockId, $field, $section[$field]);

				if ($target === 'URL')
				{
					$exportSection[$target] = $this->appendUrlUtm($exportSection[$target], $section);
				}
			}
	...
	/** @return Template\Node\Root */
	protected function utmTemplate()
	{
		return $this->once('utmTemplate', function() {
			return Template\Engine::compileTemplate((string)$this->values['UTM_TEMPLATE']);
		});
	}

	protected function needUtmCompile()
	{
		return $this->once('needUtmCompile', function() {
			$select = $this->utmTemplate()->getSourceSelect();

			return !empty($select['section']);
		});
	}

	protected function appendUrlUtm($url, array $section)
	{
		if ($this->needUtmCompile())
		{
			$entity = new Template\Entity\SourceValue($section['ID']);
			$entity->setFields(['section' => $section]);

			$utm = $this->utmTemplate()->process($entity);
		}
		else
		{
			$utm = $this->values['UTM_TEMPLATE'];
		}

		if (!is_string($utm) || trim($utm) === '') { return $url; }
