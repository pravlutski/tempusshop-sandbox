				...
				for ($i = 0, $intNFCount = count($arNeedFields); $i < $intNFCount; $i++)
				{
					...
					if (!$bFieldOut)
					{
						if (mb_substr($arNeedFields[$i], 0, mb_strlen("IP_PROP")) == "IP_PROP")
						{
							...
							{
								if(is_array($res1["PROPERTY_".$strTempo."_VALUE"])){
									$t = implode(" ", $res1["PROPERTY_".$strTempo."_VALUE"]);
								}else{
									$t = $res1["PROPERTY_".$strTempo."_VALUE"];
								}
								$arResFields[$i] = $t;
								//$arResFields[$i] = $res1["PROPERTY_".$strTempo."_VALUE"];
								//prent($res1["PROPERTY_".$strTempo."_VALUE"]);
							}
							$bFieldOut = True;
						}
