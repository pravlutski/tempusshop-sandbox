...
if (CModule::IncludeModule("catalog"))
{
	...
	if($profile_id == 17){
		$ar = CProSet::getFullOption("YMARKET_RU");
		if($ar["value"] == "START") die("Vigruzka RU zapuscnena");
		CProSet::setOption("YMARKET_RU", "START");
	}elseif($profile_id == 22){
		$ar = CProSet::getFullOption("YMARKET_BY");
		if($ar["value"] == "START") die("Vigruzka BY zapuscnena");
		CProSet::setOption("YMARKET_BY", "START");
	}
	...
	CLog::add2log(array("event" => "Y", "text" => "Выгрузка яндекса. Обработан профиль " . $ar_profile["ID"] . " " . $ar_profile["NAME"]));
}