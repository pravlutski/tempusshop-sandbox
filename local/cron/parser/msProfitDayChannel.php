...
$filepath = $_SERVER["DOCUMENT_ROOT"] . "/upload/ms/profit_channel.csv";
$fileProgress = $_SERVER["DOCUMENT_ROOT"] . "/local/cron/parser/progress_2.lock";

$settings = [];
// смотрим настройки 
$strSql = "SELECT * FROM bq_exchange WHERE ID = '2'";
...
if(!$settings["COLUMN"]){
	$settings["COLUMN"] = BQ_EXCHANGE[2]["COLUMN"];
}
...
foreach($dates as $date){
	//if($date != "2023-03-31") continue;
	$file = "/home/bitrix/logs/ms/profit_channel/{$date}.txt";
	if(file_exists($file)){
