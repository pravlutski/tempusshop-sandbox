...
class BqParse{
	...
	private function sendBigQuery($arData = []){
		...
		$schema = $this->bqTableFields($this->report["SETTINGS"]["BQ_TABLE"]);
...
// удаляем все значения в диапазоне дат
		/*if($this->bqDeleteData() === false){
			$this->LAST_ERROR = "Данные из таблица '{$this->report["SETTINGS"]["BQ_TABLE"]}' не удалось удалить";
			return false;
		}*/
		
		// проходим по всем дням
		foreach($arData as $date => $arItem){
			$tableId = "{$this->BQ_PROJECT_ID}{$this->BQ_DATASET}{$this->report["SETTINGS"]["BQ_TABLE"]}\${$date}";
			$tableId = "{$this->report["SETTINGS"]["BQ_TABLE"]}";
			$table = $this->bqDataset->table($tableId);
			
			$ar = $this->bqPrepareData($arItem, $schema, $date);
			$ar = array_slice($ar,0,1);
			//prent($ar);die;
			$this->bqInsertData2($table, $ar);
		...
		/*
		// пишим новые данные
		$arData = $this->bqPrepareData($arData, $schema);
		//prent($arData);
		$this->bqInsertData($arData, $schema);
		*/
	...
	private function bqPrepareData2($arData, $schema){
		$items = [];
	...
	private function bqPrepareData($arData, $schema){
		$items = [];
		foreach($arData as $arItem){
			$ar = [];
			foreach($schema as $sc){
	...
	private function bqInsertData2($table, $arData = [], $suffix = ""){

		$insertResponse = $table->insertRows($arData, ["templateSuffix" => $suffix]);
	...
	private function bqInsertData($arData = [], $schema){
		if(!$arData) return false;
		
		file_put_contents($this->report["FILE_PROGRESS"], "in_process#80#Отправляем массив в BQ");
		...
		$loadConfig = $this->bqTable->loadFromStorage($gcsUri)->schema($bqSchema)->sourceFormat('NEWLINE_DELIMITED_JSON');
		$job = this->bqTable->runJob($loadConfig);
...
if (isset($job->info()['status']['errorResult'])) {
	...
	private function bqTableFields($name = ""){
		$this->bqTable = $this->bqDataset->table($name);
		
		if($this->bqTable->exists()){
			$fields = $this->bqTable->info()["schema"]["fields"];
			return $fields;
		}
		return false;
	}
